...
5. Run the versioned fixed-seed impairment manifest for delay, loss, duplication and reorder with exact run length, event counts, repair deadlines and final hash equality.
...
- Authority: host/client hashes and canonical exactly-once counts must converge after repair; stale life/epoch/revision/action data fails closed.
