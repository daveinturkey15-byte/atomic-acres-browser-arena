...
3. `docs/PASS65_REQUIREMENTS_MATRIX.md` and `docs/PASS65_DECISION_RECEIPTS.json`.
4. The affected technical contract, canonical catalog, project skill and owner HITL section.
...
4. Record explicit supersessions when current feedback changes an older interpretation. Do not leave contradictory rules active.
5. Inspect the real implementation and tests. Treat code presence, a changelog claim and a green unrelated test as unknown until the row's falsifier is exercised.
6. Update the canonical source of truth before downstream consumers. Weapons, grenades, killstreaks, renderer features, interactions and evidence inventories must project automatically or fail synthetic add/rename/retire mutations.
7. Implement the smallest coherent owned change. Never create another raw `F` listener, hand-maintained content mirror, no-op graphics setting, presentation-only collision, or client-authored combat result.
8. Run `npm run qa:pass65:owner-feedback`, the affected domain gates, and the exact runtime/browser/hardware/network evidence named by each row.
9. Move states only with evidence: `OPEN` means not implemented, `IMPLEMENTED` means code exists, `VERIFIED` means mechanical falsifiers passed at the stated source identity, and `HITL` means the immutable candidate is ready for Dave's taste/owner decision. Publication remains a separate explicit gate.
