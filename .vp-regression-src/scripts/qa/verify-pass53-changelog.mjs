...
try {
  for (const profile of [
    ...
    url.searchParams.set('seed', `pass53-changelog-${profile.name}`);
    ...
    const screenshot = `${screenshotDir}/pass53-changelog-${profile.name}.png`;
    await page.screenshot({ path: screenshot, fullPage: false });
    reports.push({ profile: profile.name, viewport: profile.viewport, button, errors, state, screenshot });
    if (button !== 'LAST RELEASE · 22 JUL 2026 · 09:27 BST') throw new Error(`${profile.name}: unexpected button ${button}`);
    if (errors.length || state.entryCount !== 9 || state.firstId !== 'pass53' || state.firstDatetime !== '2026-07-22T09:27:28+01:00'
      || state.liveBadge !== 'CURRENT LIVE' || !state.firstTime.includes('BST · UTC+1 · 09:27:28') || state.horizontalOverflow
