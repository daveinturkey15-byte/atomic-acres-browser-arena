...
- Owner clarification 2026-09-12 evening: use neon colours on menu headings and selected controls with neutral body text. Replace visible project/release boilerplate with build number, build timestamp and hover/focus changes. Lobby and game chat are disabled. Remove killstreak preview; use icon choice buttons. New defaults are Care Package, Piloted Drone, Carpet Bomber, Chopper Gunner and Drone Swarm; preserve custom loadouts and migrate only the previous exact default. These explicit requests supersede historical preview/chat/default presentation requirements.
...
- Owner supersession 2026-09-12: menus use dark surfaces with restrained neon blue, green, purple, orange, yellow and red accents. The earlier bright-menu requirement is retired; combat HUD styling and gameplay remain unchanged by this menu pass. Hide all map preview surfaces and leaderboard access, and present Technique Showcase as a square tile alongside the map selector. At 1280x720, menu labels and critical HUD status text are at least 9px, primary actions and values are at least 12px, and every review viewport must be free of clipping and surface overlap.
