...
- Every new owner-feedback statement receives one stable `HF-###` ledger row before implementation, with one owner lane, affected maps/modes, a mechanical falsifier, required evidence and planning-requirement mapping. Chat acknowledgement is not coverage. `npm run qa:pass65:owner-feedback` must reject duplicate, skipped, malformed or unowned rows; `npm run qa:pass65:owner-feedback:candidate` must additionally reject every `OPEN` or merely `IMPLEMENTED` P0/P1 row. `IMPLEMENTED` is not `VERIFIED` and owner taste remains `HITL` only after mechanical gates pass.
...
- Interaction key `F` is resolved once per frame by the shared priority arbiter. An active possessed support platform owns enter/exit before nearby care packages, doors, weapons or other world prompts; otherwise exactly one eligible action and prompt wins by explicit priority, range, LOS and deterministic tie-break. Individual features may not register competing raw `F` handlers.
