import { $i as AnimationMixer, Ac as MathUtils, Bu as Ray, Cc as LoopOnce, Fa as CircleGeometry, Fc as Mesh, Fl as PlaneGeometry, Fs as InterleavedBufferAttribute, Go as Euler, Hc as MeshPhongMaterial, Hd as TextureLoader, Ic as MeshBasicMaterial, Io as DynamicDrawUsage, Jl as Quaternion, Ju as RepeatWrapping, Ka as ConeGeometry, Kd as TorusGeometry, Ml as Path, Nc as Matrix4, Nl as PerspectiveCamera, Ra as Color, Sa as BoxGeometry, Ta as BufferGeometry, Tc as LoopRepeat, Uc as MeshPhysicalMaterial, Vd as Texture, Wc as MeshStandardMaterial, Zi as AnimationClip, Zo as Float32BufferAttribute, _a as Bone, _d as SphereGeometry, _f as Vector3, ba as Box3, cd as ShapeGeometry, hs as Group, i as mergeGeometries, id as Scene, ja as CanvasTexture, kl as OrthographicCamera, ks as InstancedMesh, md as SkinnedMesh, mo as CylinderGeometry, n as clone, nd as SRGBColorSpace, r as RoundedBoxGeometry, ru as RGBAFormat, sd as Shape, t as MeshoptDecoder, vl as Object3D, vo as DataTexture, wa as BufferAttribute, zc as MeshLambertMaterial } from "./vendor-three-VV5gneRl.js";
...
var OPERATOR_SKIN_CATALOG = createOperatorSkinCatalog(freezeSourceDefinitions$1([
	{
		id: "default",
...
var PALETTES = Object.freeze({
	...
	symbiote: Object.freeze({
		...
		arm: Object.freeze({
			sleeve: 12429772,
			glove: 10325934,
			fingerGlove: 11048888,
			accent: 13605887,
			sleeveRoughness: .62,
			gloveRoughness: .48,
			accentMetalness: .34,
			accentEmissive: 2888255
		}),
		card: Object.freeze({
			backdropTop: 2890811,
			backdropBottom: 1116695,
			torso: 5918563,
			webbing: 2366764,
			trim: 10312928,
			visor: 13805311,
...
function isLobbySnapshot(value) {
	...
	return validHostStart && validEpochStart && snapshot.activeAtHostTimeMs === null === (snapshot.activeAtEpochMs === null) && validMatchClock && validTestDummies && validTestBayDoor;
}
/**
...
var clamp01 = (value) => MathUtils.clamp(Number.isFinite(value) ? value : 0, 0, 1);
/**
...
function materialForTeam(material, team, flattenMaterials, appearance = "team") {
	...
	if (appearance === "neon-purple" && name === "swat") {
		...
		result.color.setHex(team === 0 ? 2979970 : 11750719);
		result.emissive.setHex(team === 0 ? 399901 : 2361606);
		result.emissiveIntensity = flattenMaterials ? .34 : .14;
	} else if (name.includes("swat_black")) {
		result.color.setHex(team === 0 ? 1911085 : 3154726);
		result.emissive.setHex(team === 0 ? 397587 : 1246984);
		result.emissiveIntensity = flattenMaterials ? .22 : .08;
	} else if (name.includes("grey")) result.color.setHex(team === 0 ? 7183262 : 12157558);
...
*/
function createOperatorInstanceMaterialResolver(team, flattenMaterials, appearance = "team") {
...
var FIRST_PERSON_ARM_SKIN_CONTRACT = "palette-tinted-authored-arm-atlas-v1";
/**
...
function switchBaseAction(runtimeState, name) {
	...
	if (!next) return;
	next.reset().setLoop(LoopRepeat, Infinity).fadeIn(.14).play();
	previous?.fadeOut(.14);
	runtimeState.currentBase = name;
}
function playOneShot(runtimeState, name, timeScale = 1) {
...
function updateRiggedOperator(root, speed, stance) {
	const runtimeState = runtime$1(root);
	...
	const next = stance !== "stand" ? "Idle_Gun_Pointing" : speed > 3.2 ? "Run_Shoot" : speed > .18 ? "Walk" : "Idle_Gun_Pointing";
	switchBaseAction(runtimeState, runtimeState.clips.has(next) ? next : speed > .18 ? "Run" : "Idle_Gun");
	runtimeState.mixer.update(dt);
...
function reactRiggedOperator(root, alternate = false) {
	const runtimeState = runtime$1(root);
	if (!runtimeState) return false;
...
function deathRiggedOperator(root) {
	...
	for (const action of runtimeState.actions.values()) action.fadeOut(.04);
...
function applyShedStructuralBlast(definition, state, request) {
	...
	if (request.blastClass === "carpet-bomber-obliteration") {
		const clampSpeed = (value, maximum) => Math.max(-maximum, Math.min(maximum, value));
		const thrown = majorDebris.map((body) => {
			if (body.velocityQ.xQ !== 0 || body.velocityQ.yQ !== 0 || body.velocityQ.zQ !== 0) return body;
			let hash = 2166136261 ^ body.chunkId.length + 1;
			for (let index = 0; index < body.chunkId.length; index += 1) hash = Math.imul(hash ^ body.chunkId.charCodeAt(index), 16777619);
			hash >>>= 0;
			const unit = (n) => (hash >>> n & 65535) / 65535;
			const awayX = body.poseQ.position.xQ / 1e3 - request.originLocal.x;
			const awayZ = body.poseQ.position.zQ / 1e3 - request.originLocal.z;
			const away = Math.hypot(awayX, awayZ);
			const outward = away > .05 ? 1 / away : 0;
			return Object.freeze({
				...body,
				velocityQ: Object.freeze({
					xQ: Math.round(clampSpeed(awayX * outward * (3 + unit(0) * 1.4), 9) * 1e3),
					yQ: Math.round(clampSpeed(2.2 + unit(8) * 1.8, 9) * 1e3),
					zQ: Math.round(clampSpeed(awayZ * outward * (3 + unit(16) * 1.4), 9) * 1e3)
				}),
				angularVelocityQ: Object.freeze({
					xQ: Math.round((unit(24) * 2 - 1) * 4.2 * 1e3),
					yQ: Math.round((unit(32) * 2 - 1) * 1.6 * 1e3),
					zQ: Math.round((unit(40) * 2 - 1) * 4.2 * 1e3)
				}),
				flat: false,
				sleeping: false
			});
	...
	} else surfaces = Object.freeze(surfaces.map((surface) => surface.stage === "intact" ? Object.freeze({
		...surface,
...
function impulseMajorShedDebris(state, request) {
	...
	return {
		...
		state: withRevision$1(state, { majorDebris: Object.freeze(state.majorDebris.map((candidate) => candidate.chunkId === request.chunkId ? Object.freeze({
			...candidate,
			sleeping: false,
			velocityQ: Object.freeze({ ...request.impulseQ })
		}) : candidate)) })
	};
}
...
function localToWorld(house, x, y, z) {
...
function houseFurnitureFragment(house) {
	...
	return Object.freeze({
		...
		position: localToWorld(house, side * 6.75, .82, -5.65),
		halfExtents: frozenPoint(.62, .82, .36),
		rotation: Object.freeze({
			x: 0,
			y: 0,
			z: 0,
...
*/
var HOST_SUCCESSION_MANDATE_TTL_MS = REJOIN_GRACE_MS;
...
function buildRetroCoach() {
	...
	part(root, roundedBox("coach-body", [
		5.3,
		3.45,
		13.6
	], new MeshStandardMaterial({
	...
	for (const side of [-1, 1]) for (let z = -4.8; z <= 4.8; z += 2.4) part(root, roundedBox("coach-window", [
		.055,
		1.34,
		1.85
	], glass, .08), [
	...
	for (const x of [-1.8, 1.8]) for (const z of [-4.6, 4.6]) wheel(root, x, z, .74);
	for (const x of [-1.75, 1.75]) {
		const light = new Mesh(new CircleGeometry(.26, 20), new MeshStandardMaterial({
			color: 16773298,
			emissive: 16758861,
			emissiveIntensity: 2.3
		}));
		light.position.set(x, 1.55, -6.88);
		root.add(light);
	}
	const sign = decal("ATOM-LINER 86", 3.6, .9);
	sign.position.set(0, 3.25, -6.9);
	root.add(sign);
	return root;
}
function buildRetroShuttleBus() {
	const root = new Group();
	root.name = "original-acres-shuttle-bus";
	part(root, roundedBox("shuttle-body", [
		4.9,
		3.45,
		10.6
	], texturedMaterial("./assets/original/textures/painted-metal-teal.png", {
		color: 5218209,
		roughness: .5,
		metalness: .24,
		normalPath: "./assets/original/textures/painted-metal-teal-normal.png",
	...
	part(root, roundedBox("shuttle-lower-skirt", [
		5.02,
		.7,
		10.2
	], MAT.dark(), .18), [
	...
	part(root, roundedBox("shuttle-roof", [
		4.62,
		.32,
		9.95
	], MAT.cream(), .15), [
	...
	const glass = MAT.glass();
	part(root, roundedBox("shuttle-windshield", [
		3.9,
		1.32,
		.08
	], glass, .08), [
	...
	part(root, roundedBox("shuttle-rear-glass", [
		3.75,
		1.08,
		.08
	], glass, .08), [
	...
	for (const side of [-1, 1]) {
		for (const z of [
			...
			1.05,
			3.35
		]) part(root, roundedBox("shuttle-window", [
			.055,
			1.24,
			1.55
		], glass, .07), [
		...
		const door = roundedBox("shuttle-passenger-door", [
			.065,
			2.38,
			1.55
		], glass, .06);
		...
		root.add(door);
		part(root, roundedBox("shuttle-mirror-arm", [
			.5,
			.08,
			.08
		], MAT.dark(), .02), [
		...
		part(root, roundedBox("shuttle-mirror", [
			.1,
			.4,
			.28
		], glass, .04), [
	...
	for (const x of [-1.72, 1.72]) for (const z of [-3.45, 3.35]) wheel(root, x, z, .7);
	part(root, roundedBox("shuttle-front-bumper", [
		5.02,
		.34,
		.34
	], MAT.dark(), .08), [
	...
	part(root, roundedBox("shuttle-rear-bumper", [
		5.02,
		.34,
		.34
	], MAT.dark(), .08), [
	...
	const sign = decal("NUKE SHUTTLE", 3.5, .72);
	sign.position.set(0, 3.18, -5.46);
	root.add(sign);
	return root;
}
...
function poseOperator(root, stance, speed, _phase, _blend = 1, _aimPitch = 0, explicitDeltaSeconds) {
	const rig = operatorRig(root);
	if (!rig) return;
	const now = performance.now();
...
//#endregion
export { hitProxyRootTransform as $, freeForAllLeaders as $i, PASS65_KILLSTREAK_SLOT_DEFINITIONS as $n, applyFirstPersonArmSkin as $r, flarePresentationReplicaKey as $t, reloadProgress as A, restoreGunRangeMatchClock as Aa, damp as Ai, canAdmitMajorDebris as An, HIGH_SCORE_STORAGE_KEY as Ar, isGuestCombatInventory as At, fireOperator as B, interpolateHostedBotSnapshot as Ba, scheduleBrowserPreparationIdleTask as Bi, CARPET_BOMBER_RESIDUAL_FIRE_DURATION_MS as Bn, configureRuntimeRandom as Br, authorizeSelfPromotion as Bt, integrateHorizontalVelocity as C, advanceGunRangeMatchClock as Ca, measureCameraFraming as Ci, SMOKE_CORRIDOR_RADIUS_M as Cn, admitChatRate as Cr, updateImportedWeapon as Ct, nextStance as D, holdGunRangeReplicaAtAuthorityBoundary as Da, proneBodyClearance as Di, HOUSE_DESTRUCTION_DEFINITION_SET_ID as Dn, normalizeChatText as Dr, SPECIAL_WEAPON_IDS as Dt, movementProfile as E, gunRangeTestBayOccupants as Ea, isSharedMeshGeometry as Ei, InteractiveWorldRuntime as En, normalizeChatSenderName as Er, SIDEARM_WEAPON_IDS as Et, buildRetroCoach as F, isDhv as Fa, segmentIntersectsBox as Fi, CarpetGroundFireGuestPresentationAdmission as Fn, normalizeRequiredPlayerName as Fr, GRENADE_CATALOG as Ft, resetOperator as G, parseWeaponDefinitions as Ga, CLOCK_PING_INTERVAL_MS as Gi, isKillstreakRuntimeCheckpoint as Gn, runtimeSeed as Gr, isSuccessionRoster as Gt, optimizeAttachedWeapon as H, isHostedBotSnapshot as Ha, yieldBrowserCpuTask as Hi, HostKillstreakRuntime as Hn, presentationRandom as Hr, evaluateHostLoss as Ht, buildRetroShuttleBus as I, reportedDhvRawDamage as Ia, sphereIntersectsBox as Ii, carpetGroundFireStateChunks as In, peerOwnedHighScores as Ir, GRENADE_IDS as It, texturedMaterial as J, REJOIN_GRACE_MS as Ji, applyPilotedDroneScreenLookDelta as Jn, resetMinigunSpool as Jr, resolveRoomClaimOutcome as Jt, roundedBox as K, isArenaId as Ka, DEFAULT_PRIVATE_MATCH_CONFIG as Ki, resolveSupportAircraftEnvelopeStep as Kn, advanceMinigunSpool as Kr, mintSuccessionMandate as Kt, buildWeaponModel as L, hostedBotIds as La, sweepSphereAgainstBoxes as Li, CARPET_BOMBER_BLAST_RADIUS_M as Ln, personalBest as Lr, HOST_SUCCESSION_MANDATE_TTL_MS as Lt, sprintEligible as M, applyDhvIncomingDamage as Ma, isBlocked as Mi, admitKillstreakCareCaptureResultMessage as Mn, leaderboardNameKey as Mr, isPlayerSnapshot as Mt, batchStaticMeshes as N, applyDhvWeaponOutgoingDamage as Na, pointInsideBounds as Ni, admitKillstreakStateMessage as Nn, loadHighScores as Nr, isStateTrafficMessage as Nt, recoverRecoil as O, isGunRangeMatchClockSnapshot as Oa, clampPointToBounds as Oi, createAtomicHouseFragmentDefinitions as On, isReservedMultiplayerParticipantId as Or, WEAPON_IDS as Ot, buildOperator as P, dhvLabel as Pa, resolveHorizontalMove as Pi, CARPET_GROUND_FIRE_AUTHORITY_CAPACITY as Pn, mergeHighScores as Pr, messageBelongsToPlayer as Pt, AUTHORITATIVE_HIT_PROXIES as Q, emptyPlayerScore as Qi, PASS65_KILLSTREAK_CATALOG as Qn, applyBotEmissiveBrightness as Qr, compareFlarePresentationReplicaIdentity as Qt, createOperatorWeaponPresentation as R, hostedBotReplicationActive as Ra, browserOwnsForegroundPresentation as Ri, CARPET_BOMBER_IMPACT_FLASH_BASE_RADIUS_M as Rn, saveHighScores as Rr, MAX_HOST_TERM as Rt, integrateGamepadLookRate as S, reserveHudValue as Sa, characterActionContract as Si, SMOKE_COLOUR_PALETTE as Sn, railgunThermalTargetEligible as Sr, resetImportedWeaponAnimations as St, mouseSensitivityMultiplier as T, gunRangeTestBayOccupancyBoundaryCount as Ta, GPU_SHARED_GEOMETRY_KEY as Ti, SmokeAuthority as Tn, normalizeChatHistory as Tr, PRIMARY_WEAPON_IDS as Tt, poseOperator as U, WEAPON_CATALOG as Ua, yieldBrowserPreparationFrame as Ui, chopperGunnerAuthoritativeRay as Un, protocolRandom as Ur, hostLossPresentation as Ut, meleeOperator as V, isHostedBotCount as Va, waitForVisibleBrowserPreparation as Vi, CHOPPER_MISSILE_BLAST_RADIUS_M as Vn, gameplayRandom as Vr, electHostSuccessor as Vt, reactOperator as W, sustainedRecoilBurden as Wa, yieldVisibleBrowserPresentationFrame as Wi, chopperGunnerCameraOrigin as Wn, runtimeRandomTelemetry as Wr, isSuccessionMandate as Wt, damageNumberPresentation as X, canHostCommitStart as Xi, SUPPORT_FORWARD_AXIS as Xn, RIGGED_OPERATOR_CORPSE_ACTION_NAMES as Xr, MAX_FLARE_PRESENTATION_REPLICAS as Xt, waitForPendingArtTextures as Y, balanceLobbyTeams as Yi, pilotedDroneControlAxes as Yn, BOT_EMISSIVE_BRIGHTNESS_SCALE as Yr, BotWeaponPresentationReplayGuard as Yt, roundStatSummary as Z, canHostStart as Zi, CRIMSON_FLAMETHROWER_KILLSTREAK_ID as Zn, RIGGED_OPERATOR_RENDERED_INFLUENCE_THRESHOLDS as Zr, canonicalizeFlarePresentationReplicas as Zt, computeRecoilImpulse as _, isGunRange as _a, setFirstPersonArmBaseAction as _i, isTimedMapWeaponAuthorityState as _n, grantTrainingRailgun as _r, pass65WeaponCacheTelemetry as _t, PLAYER_JUMP_GRAVITY as a, recordPlayerDamage as aa, loadOperatorSkinAsset as ai, flareBurnDamagePerSecond as an, DRONE_SUPPORT_DEFINITIONS as ar, createPass65CrossbowModel as at, effectiveHitZoneForWeapon as b, reloadSupply as ba, solveTwoBoneElbow as bi, FlashVictimResultConsumer as bn, isStaleRailgunAuthorityState as br, releasePass65WeaponModelsIn as bt, advanceFreeForAllMatch as c, OPERATOR_SKIN_CATALOG as ca, playFirstPersonArmAction as ci, TIMED_MAP_WEAPON_DEFINITIONS as cn, RAILGUN_UPPER_ROOM_SPAWN_SITES as cr, disposePass65WeaponModel as ct, beginReload as d, defaultSquadPresentation as da, resetFirstPersonArmFingers as di, claimTimedMapWeapon as dn, advanceRailgunChamber as dr, invalidatePass65PresentationTree as dt, isLobbyMember as ea, createFirstPersonRiggedArms as ei, isFlarePresentationReplicaSnapshot as en, parseKillstreakLoadout as er, hitProxyZoneCentre as et, botScaledDamage as f, renderSquadRosterBadge as fa, resolveRiggedOperatorRuntimeRoot as fi, consumeTimedMapWeaponShot as fn, claimRailgun as fr, isPass65AuthoredFirearm as ft, computeFallDamage as g, hasUnlimitedRangeAmmo as ga, riggedOperatorTelemetry as gi, isStaleTimedMapWeaponAuthority as gn, fireRailgun as gr, meleeImportedWeapon as gt, computeDamage as h, advanceRangeScore as ha, riggedOperatorHandEvidenceIdentity as hi, grantTrainingTimedMapWeapon as hn, dropRailgun as hr, loadPass65WeaponPresentation as ht, MATCH_WARMUP_MS as i, playersAreHostile as ia, loadFirstPersonArmsAsset as ii, advanceFlareProjectileKinematics as in, DRONE_PRESENTATION_FAMILY_ID as ir, capturePass70FirstPersonMaterialState as it, sampleWeaponPellet as j, DHV_VALUES as ja, firstSegmentBoxHit as ji, FIELD_SHED_DEFINITION as jn, immediateStreakEntry as jr, isHostAuthorityMessage as jt, recoverRecoilImpulse as k, projectGunRangeMatchClock as ka, collidersOverlappingVerticalSpan as ki, SHARED_MAJOR_DEBRIS_BUDGET as kn, LEADERBOARD_SEASON as kr, isGameMessage as kt, advanceMatch as l, isSelectableOperatorSkinId as la, prewarmRiggedOperatorActions as li, TIMED_MAP_WEAPON_IDS as ln, admitRailgunTargets as lr, fireImportedWeapon as lt, completeReload as m, GUN_RANGE_ROUND_MS as ma, riggedOperatorCanonicalEvidenceManifest as mi, dropTimedMapWeapon as mn, createRailgunBeamAuthority as mr, loadPass65WeaponAsset as mt, DEFAULT_MATCH_RULES as n, isPrivateMatchConfig as na, firstPersonArmAnimationState as ni, FLAMETHROWER_EFFECT as nn, CHOPPER_GUN_PROFILE as nr, PASS70_FIRST_PERSON_OPTIC_WINDOW_CONTRACT as nt, WEAPONS as o, rejoinReservationExpired as oa, loadRiggedOperatorAsset as oi, flameDamageAllowsTarget as on, RAILGUN_RECHAMBER_MS as or, createPass65FieldKnifeModel as ot, cancelReload as p, sanitizeSquadPresentation as pa, riggedOperatorAssetReady as pi, createTimedMapWeaponAuthority as pn, createRailgunAuthorityState as pr, loadPass65FieldKnifeAsset as pt, setOperatorWeapon as q, LOBBY_START_LEAD_MS as qi, applyPilotedDronePointerDelta as qn, createMinigunSpoolState as qr, resolveHostTermConflict as qt, MATCH_DURATION_MS as r, latencyQuality as ra, firstPersonArmBaseActionFor as ri, FLARE_PROJECTILE_EFFECT as rn, DRONE_GUN_PROFILE as rr, PASS70_FIRST_PERSON_OPTIC_WINDOW_OPACITY as rt, admittedPlayerDamage as s, teamTotals as sa, operatorSkinAssetReady as si, flameDamagePerPulse as sn, RAILGUN_SPAWN_DELAY_MS as sr, createPass65WeaponModel as st, BOT_DAMAGE_MULTIPLIER as t, isPlayerScore as ta, createRiggedOperator as ti, isFlarePresentationStateMessage as tn, validateKillstreakLoadout as tr, PASS65_AUTHORED_WEAPON_URLS as tt, applyRadialDeadzone as u, operatorSkinPalette as ua, resetFirstPersonArmAnimations as ui, advanceTimedMapWeaponAuthority as un, advanceRailgunAuthority as ur, importedWeaponTelemetry as ut, computeSpread as v, rangeAccuracyPercent as va, updateFirstPersonArmAnimations as vi, isFlashResultMessage as vn, isRailgunAuthorityState as vr, prewarmPass65RuntimeWeaponCorpus as vt, meleeStrike as w, createGunRangeMatchClockSnapshot as wa, resolveSocketWorld as wi, SMOKE_VOLUME_LIFETIME_MS as wn, appendChatHistory as wr, ORDINARY_WEAPON_IDS as wt, grenadeDamage as x, reserveAfterCompletedReload as xa, solveTwoBoneElbowInto as xi, flashActivationId as xn, railgunStateResyncDue as xr, reloadImportedWeapon as xt, createMatch as y, rangeGrenadesAllowed as ya, updateRiggedOperator as yi, FlashHostAuthority as yn, isRailgunBeamAuthority as yr, releasePass65WeaponModel as yt, deathOperator as z, hostedBotSnapshotContinuity as za, browserPresentationIsVisible as zi, CARPET_BOMBER_IMPACT_FLASH_MAXIMUM_SCALE as zn, MAX_MATCH_KILLS as zr, acceptPromotedHost as zt };
