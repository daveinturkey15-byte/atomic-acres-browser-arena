import { Ac as MathUtils, Fa as CircleGeometry, Fc as Mesh, Fl as PlaneGeometry, Ju as RepeatWrapping, Sa as BoxGeometry, Ta as BufferGeometry, Wc as MeshStandardMaterial, Zo as Float32BufferAttribute, Zu as RingGeometry, _d as SphereGeometry, _f as Vector3, ba as Box3, dc as LinearFilter, hc as LinearMipmapLinearFilter, hs as Group, mo as CylinderGeometry, nd as SRGBColorSpace, ru as RGBAFormat, vo as DataTexture, wa as BufferAttribute } from "./vendor-three-VV5gneRl.js";
	...
	switch (family) {
		...
		case "stair":
			...
			for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
				...
				if (isCaulk) {
					...
					const r = MathUtils.clamp(Math.round(baseR * (1 + plankTone + grain) * (.85 + .15 * bevel)), 0, 255);
					const g = MathUtils.clamp(Math.round(baseG * (1 + plankTone + grain) * (.85 + .15 * bevel)), 0, 255);
					const b = MathUtils.clamp(Math.round(baseB * (1 + plankTone + grain) * (.85 + .15 * bevel)), 0, 255);
					albedoData[offset] = r;
		...
		case "wall":
			...
			for (let y = 0; y < size; y += 1) for (let x = 0; x < size; x += 1) {
				...
				const bevel = edgeDist >= 2 ? Math.min(1, (edgeDist - 2) / 4) : 0;
				const surfaceNoise = (smoothNoise2D(x * .1, y * .1, 13) - .5) * .03;
				const offset = (y * size + x) * 4;
				const pIndex = y * size + x;
				if (isSeam) {
					albedoData[offset] = MathUtils.clamp(Math.round(baseR * .7), 0, 255);
					albedoData[offset + 1] = MathUtils.clamp(Math.round(baseG * .7), 0, 255);
					albedoData[offset + 2] = MathUtils.clamp(Math.round(baseB * .7), 0, 255);
					albedoData[offset + 3] = 255;
					...
					const factor = (1 + surfaceNoise) * (.88 + .12 * bevel);
					...
					albedoData[offset + 3] = 255;
					const rough = MathUtils.clamp(Math.round((.4 + (1 - bevel) * .18) * 255), 0, 255);
					roughnessData[offset] = rough;
					roughnessData[offset + 1] = rough;
					roughnessData[offset + 2] = rough;
					roughnessData[offset + 3] = 255;
					heightData[pIndex] = .3 + .65 * bevel + surfaceNoise * .05;
				}
			}
			break;
		case "roof":
...
*/
var BELOW_DECK_PRACTICAL_EMISSIVE_INTENSITY = 3.4;
...
function addCabin(builder, end, wallMaterial, deckMaterial, roofMaterial, stairMaterial, trimMaterial, glassMaterial) {
	...
	for (const [side, x, glassRotation] of [[
		...
		for (const zOffset of [
			-4.5,
			0,
			4.5
		]) detailBox(builder, `high-seas-${end}-upper-${side}-window-${zOffset}`, [
			x,
			7.55,
			centerZ + zOffset
		], [
...
function addRails(builder, railMaterial, deckMaterial) {
	...
	for (let z = -40; z <= 40; z += 5) {
		...
		detailBox(builder, `high-seas-starboard-stanchion-${z}`, [
			10.26,
			4.42,
			z
		], [
	...
	for (const z of [
		...
		4,
		8
	]) detailBox(builder, `high-seas-catwalk-stanchion-${z}`, [
		-11.66,
		4.42,
		z
	], [
...
function buildHighSeas(scene) {
	...
	const deckMaterial = material("honey-deck", 12027475, .7, .08);
	const stairMaterial = material("dark-deck-stair", 5914674, .76, .08);
	const tealTrimMaterial = material("deep-teal-trim", 1461336, .32, .62);
	const engineWallMaterial = applyEnclosedVolumeFill(material("engine-bulkhead", 3557196, .52, .58), BELOW_DECK_FILL.bulkhead);
	const engineFloorMaterial = applyEnclosedVolumeFill(material("engine-grating", 2504250, .46, .72), BELOW_DECK_FILL.grating);
	const engineMachineMaterial = applyEnclosedVolumeFill(material("engine-machinery", 5727850, .38, .74), BELOW_DECK_FILL.machinery);
	...
	const glassTextures = generateMaterialTextureSet("glass", 6200488);
