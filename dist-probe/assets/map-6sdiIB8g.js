...
import { Fc as Mesh, Fl as PlaneGeometry, Ic as MeshBasicMaterial, Jo as ExtrudeGeometry, Ma as CapsuleGeometry, Ml as Path, Sa as BoxGeometry, Uc as MeshPhysicalMaterial, Wc as MeshStandardMaterial, _d as SphereGeometry, _f as Vector3, ba as Box3, hs as Group, ja as CanvasTexture, mo as CylinderGeometry, nd as SRGBColorSpace, sd as Shape } from "./vendor-three-VV5gneRl.js";
import { Dn as HOUSE_DESTRUCTION_DEFINITION_SET_ID, J as texturedMaterial, On as createAtomicHouseFragmentDefinitions } from "./gameplay-D7mQKMV7.js";
import { _ as NEIGHBOURHOOD_BIN_COLLIDER_SIZE, b as SPAWN_LAYOUT, c as createBallisticSurface, d as ARENA_BOUNDS, f as COVER_LAYOUT, g as NEIGHBOURHOOD_BENCH_LAYOUT, h as NEIGHBOURHOOD_BENCH_COLLIDER_SIZE, m as HOUSE_LAYOUT, n as classifyImpactSurface, p as GARAGE_LAYOUT, v as NEIGHBOURHOOD_BIN_POSITIONS, y as PATROL_LAYOUT } from "./combat-feedback-bO2zzrSz.js";
...
var CharacterPhysics = class CharacterPhysics {
	...
	static async create(colliders, bounds, safetyFloorY = 0) {
		const { default: RAPIER } = await __vitePreload(async () => {
			const { default: RAPIER } = await import("./rapier-hgFWE0Fr.js");
	...
	*/
	prewarmMajorDebrisBodies(entries) {
		const ids = entries.map((entry) => entry.id);
		if (entries.length > 18 || new Set(ids).size !== ids.length || ids.some((id) => !/^[a-z0-9][a-z0-9:-]{0,127}$/.test(id)) || entries.some((entry) => ![
			entry.halfExtents.x,
			entry.halfExtents.y,
			entry.halfExtents.z
...
function buildArena(scene) {
	...
	const palette = {
		...
		chrome: material(11451841, .23, .76),
	...
	function box(name, position, size, mat, solid = true, cast = true, blocksShots = solid, ballisticMaterial, breakableWindowId) {
	...
	function collisionProxy(name, position, size) {
		const proxy = box(name, position, size, palette.dark, true, false);
		proxy.visible = true;
		proxy.userData.collisionProxy = true;
	}
	function authoredCollisionProxy(name, position, size, ballisticMaterial = "structural-metal") {
	...
	function addPerformanceLargeCover(id, x, z) {
		...
		if (id === "north-cargo-stack") {
			for (const offset of [-1, 1]) addBox("performance-cargo-lower", [
				x + offset,
				.52,
				z
			], [
			...
			for (const offset of [-.82, .82]) addBox("performance-cargo-lock-rail", [
				x + offset,
				1.62,
				z - .93
			], [
		...
		if (id === "west-service-skip") {
			...
			for (const offset of [-1.35, 1.35]) addBox("performance-skip-side", [
				x + offset,
				1.02,
				z
			], [
			...
			for (const offset of [-1.35, 1.35]) addBox("performance-skip-top-rail", [
				x + offset,
				1.92,
				z
			], [
		...
		for (const wheelX of [-1.34, 1.34]) for (const wheelZ of [-1.08, 1.08]) addCylinder("performance-generator-wheel", [
			x + wheelX,
			.48,
			z + wheelZ
		], .38, .24, palette.dark, [
	...
	const ground = new Mesh(new PlaneGeometry(86, 98), palette.grass);
	...
	const road = new Mesh(new PlaneGeometry(19, 88), palette.road);
	...
	for (const x of [-10.25, 10.25]) box("curb", [
		x,
		.12,
		0
	], [
	...
	for (const x of [-12.6, 12.6]) box("sidewalk", [
		x,
		.07,
		0
	], [
	...
	for (let z = -38; z <= 38; z += 8) box("lane marker", [
		0,
		.055,
		z
	], [
	...
	for (const z of [-18, 18]) for (let x = -7.5; x <= 7.5; x += 2.5) box("crosswalk stripe", [
		x,
		.062,
		z
	], [
	...
	for (const z of [2, 12]) for (const x of [-5.7, -1.9]) {
		const wheel = new Mesh(new CylinderGeometry(.65, .65, .38, 8), palette.dark);
		wheel.rotation.z = Math.PI / 2;
		wheel.position.set(x, .7, z);
		world.add(wheel);
	}
	box("south shuttle bus", [
		4.2,
		1.75,
		-8.8
	], [
	...
	physicalCover.push({
		id: "south-shuttle-bus",
		bounds: { ...colliders[colliders.length - 1] },
		blocksMovement: true,
		blocksShots: true
	});
	box("shuttle bus roof", [
		4.2,
		3.62,
		-8.8
	], [
	...
	box("garage door", [
		...
		2.7,
		.18
	], palette.chrome, true, false, true);
	box("garage door", [
		southGarage.x,
		1.55,
		southGarage.z - 3.3
	...
	for (const bank of [
		...
		const visual = new Mesh(new SphereGeometry(1, 16, 8), palette.grass);
		...
		const bankColliders = bank.slices.map(([x, width, depth, height], index) => {
			...
			const proxy = authoredCollisionProxy(name, [
				x,
				height / 2,
				bank.position[2]
			], [
		...
		qualityEarthBankAudit.push({
			id: bank.id,
			colliders: bankColliders
		});
	}
	const vesselCollider = box("east-irrigation-vessel-collider", [
		27,
		1.65,
		28
	], [
		...
		3.3,
		3.8
	], palette.chrome, true, false, true, "structural-metal");
	vesselCollider.visible = false;
	vesselCollider.userData.collisionAuthorityFor = "east-irrigation-vessel";
	const irrigationVessel = new Mesh(new CylinderGeometry(1.9, 1.9, 3.3, 20), palette.chrome);
	...
	world.userData.atomicCollisionAudit = {
		...
		largeCylinder: {
			id: irrigationVessel.name,
			collider: vesselCollider.name,
			bottomY: 0
		}
	};
	const authoredLargeCoverIds = /* @__PURE__ */ new Map([
		[4, "north-cargo-stack"],
		[5, "south-pipe-stack"],
		[6, "west-service-skip"],
	...
	for (const x of [-29, -22]) for (const z of [-15, -5]) collisionProxy("skyline trellis column", [
		x,
		1.9,
		z
	], [
	...
	collisionProxy("service wall west", [
		22.5,
		.75,
		9
	], [
	...
	collisionProxy("service wall east", [
		28.5,
		.75,
		9
	], [
	...
	for (const x of [22.5, 29.5]) for (const z of [-20, -12]) collisionProxy("solar canopy column", [
		x,
		2.1,
		z
	], [
	...
	for (const [index, [x, z]] of [
		...
		[-22, -24],
		[22, 25]
	].entries()) substantial(`authored-terminal-collider-${index}`, [
		x,
		.85,
		z
	], [
	...
	for (const [index, [x, z]] of [[-29, 4], [29, -4]].entries()) substantial(`authored-extra-lamp-collider-${index}`, [
		x,
		2.8,
		z
	], [
	...
	for (const [index, x] of [
		...
		-25.5,
		-23
	].entries()) substantial(`authored-hydro-bed-collider-${index}`, [
		x,
		.35,
		16
	], [
	...
	substantial("authored-reclamation-tank-collider", [
		-31,
		3.05,
		4
	], [
	...
	for (const [index, x] of [-11.3, 11.3].entries()) substantial(`authored-civic-post-collider-${index}`, [
		x,
		3.25,
		0
	], [
...
//#endregion
export { CharacterPhysics as a, solidBounds as c, pass73CollisionRouteFixtures as i, assertPass73CollisionRouteAuthority as n, worldBoundaryColliders as o, auditPass73CollisionRouteAuthority as r, createHouseArchitecture as s, buildArena as t };
