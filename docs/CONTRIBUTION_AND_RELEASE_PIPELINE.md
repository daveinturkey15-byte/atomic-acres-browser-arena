...
### Pass 71 candidate-A HF-298 grenade native coverage

The hosted Windows `software-ci-semantic` grenade shard is acceptance-ineligible. After the immutable preview for final candidate A exists, check out that exact clean A and run the release-owned installed-Edge gate with its SHA explicitly supplied. The finalizer runs the exact representative matrix `solo|hosted x webgl2|webgpu` at Atomic Acres in Performance profile:
...
The runner rejects source dirt before or after the run, a checkout or staged-source mismatch, Vite overrides, a non-Edge browser, backend drift, a software adapter, device loss, browser/runtime faults, any missing cold/warm Frag/Flash/Smoke/Semtex trial, and every backend-appropriate action-frontier breach. WebGPU requires real submission/completion advancement; synchronous WebGL2 requires exact zero submission sequences and does not invent a GPU queue frontier. Hosted trials create an owned PeerJS process and prove a real two-member private lobby before the host throws. The pinned npm lockfile preflight runs without claiming `npm ci`; Edge's on-disk ProductVersion, install root, executable hash and valid Microsoft Authenticode identity must agree with the observed `Edg/` UA/version. Each grenade gets a separate topology-wrapper and Playwright invocation. That wrapper requires an unbound port, creates a temporary build, stages the release topology, and starts Vite with `strictPort`, so the cold action starts in a fresh owned staged preview, Edge process, profile and GPU-cache boundary; the warm action remains in that trial. Unlike the software shard it retains and gates maximum rAF as well as handler, frame-work, next-rAF, submission, completion and pending frontiers. `servedSourceSha` comes from a no-cache browser fetch of the staged `channels/the-big-one/channel-provenance.json`; it is not copied from `SOURCE_SHA` or another runner variable.

A pass writes ignored local diagnostics under `artifacts/pass71/grenade-native/`: four scope component receipts, one coverage receipt, file hashes, and `<candidate-A-sha>-hf298-native-evidence.json`. A local artifact path or `artifact://` string is not CI-verifiable release evidence. The last file is the ready-to-paste five-record array: four strict component objects followed by one coverage object that binds their canonical digests and timestamps. Every `receiptSha256` is recomputed from the embedded object itself, excluding only that digest field. Before finalization the complete array can be independently checked with:

```powershell
npm run qa:pass71:grenade-native:verify -- --evidence artifacts/pass71/grenade-native/$candidateSha-hf298-native-evidence.json --expected-source-sha $candidateSha
```

After candidate A's immutable preview exists, candidate B changes only `acceptance/pass-71.json`. Copy the complete array unchanged into the manifest's top-level `nativeEvidence` array and map HF-298 explicitly with `requirements[2].id = "R3"` and `requirements[2].feedbackId = "HF-298"`. A verified R3 requires exactly the four representative component scopes plus the one canonical coverage record; a lone solo WebGPU component cannot close HF-298. The array is an evidence-validator registry boundary rather than a globally fixed five-record set, so separately registered HF-296/297/302/303/310/311 validators can coexist without weakening HF-298. Duplicate scopes, missing/reordered coverage inputs, unregistered IDs/kinds, unknown schema fields, transcription drift and stale candidate-A hashes for the runner, contracts, spec, frame budget, PeerJS support, topology, Playwright/Vite configuration, release channels, acceptance gate, package manifest/lock, lock verifier, and browser identity helpers fail. CI and the production workflow both run `acceptance-gate.mjs`, which recomputes every embedded digest and measurement eligibility, requires expected/checkout/served/end SHA to equal the manifest preview's candidate A SHA, enforces each component and finalizer between preview creation and human approval, hashes those unchanged candidate-A blobs with `git show`, and separately proves candidate A is an ancestor with an exact candidate-B delta of only `acceptance/pass-71.json`. Adding a kind requires a named exact-schema validator rather than an opaque external reference.
