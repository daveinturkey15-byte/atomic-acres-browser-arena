...
The promoted Pass 73 root chooser will expose five builds:
...
On the promoted public root, room invitations and `release=latest`, `normal` or `experimental` select Pass 73; `previous` or `pass72` select Pass 72; `pass70` selects Pass 70; `pass69` selects Pass 69; and `stable` or `rollback` select Pass 63. Pass 67.1 has no chooser card or legacy alias.
