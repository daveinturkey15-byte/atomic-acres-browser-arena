...
| HF-109 | P0 | The owner's repeated report that preview videos still showed the wrong/non-real level keeps actual selected-arena media OPEN until visual evidence proves it. | Preview-media forge/independent QA | Every selected map is identified by multiple unique real-map landmarks in the decoded checked-in video and rapid switching never shows another map. | All four arena previews | OPEN |
...
### 3A. Known remote-attachment residual

The current audit found that a remote Semtex or explosive-crossbow hit still receives `hit.stuck=true` from the sender. Admission correlates and caps the event, but the receiver does not independently prove actor attachment from authoritative geometry and the current target life. The frozen desired multipliers and `STUCK` presentation remain the contract, not completion evidence; HF-100, HF-101 and HF-120 therefore remain `OPEN` until the remote-admission falsifiers pass. HF-102 records the implemented victim-alert presentation only and does not attest the missing attachment proof.
