...
- While a support control session is available, overlap its `F` toggle with every door/crate/weapon prompt. Exactly one prompt/action appears and support enter/exit wins; after exit the next eligible world action works on a fresh key edge.
