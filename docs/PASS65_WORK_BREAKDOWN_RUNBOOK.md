...
| Task | Owner | Dependencies | Outcome | Requirements | Smallest validation | Exit evidence |
|---|---|---|---|---|---|---|
| P01 | Spec owner | G05,G06 | Add repo-local numbered Pass 65 spec mapped to all 99 requirements/16 decisions. | R005 | Acceptance schema lints. | Spec digest and mapping report. |
...
| P04 | Decision owner | P01 | Maintain canonical `docs/PASS65_DECISION_RECEIPTS.json` against its schema. P0 historically instantiated DEC-01…DEC-15 as `OPEN`; Dave froze them on 2026-07-26, and the normalized 2026-07-27 Desktop feedback adds frozen DEC-16 and supersedes DEC-04/07/08/13. Only the current validated structured receipt and canonical digest satisfy `P04[DEC-x=FROZEN]`. | R005 | Schema, ID/status/field/dependency/digest linter rejects missing/duplicate/OPEN receipts, bad supersession, incomplete slot families and stale care-weight formula. | Validated product-decision registry and digest. |
...
| K05 | Bomber owner | A06,DW01,P04[DEC-13=FROZEN] | Use the Care Package-style crosshair arm/preview/commit lifecycle, large shared red X, caller-only red corridor and actual inbound aircraft; host-seeded RNG selects a random valid ingress and exactly 20 pooled zigzag bombs stay inside the arena-relative strip bounds. | R505,R511 | Target/commit/cancel/consume, two-peer marker/corridor/aircraft and seeded ingress/path/schedule/count/bounds/occlusion tests. | Bomber evidence. |
...
| K09 | Swarm owner | K08,A05,P04[DEC-13=FROZEN] | Exactly 24, deterministic valid centre-map spawn with ≥1.15m separation, fast ingress, distributed divergent individual/small-group routes, 20-round unlimited reload loops, ≤60s, indoor/outdoor acquisition of eligible opposing living players and bots, and frozen approximately-five-second exposure/escape survival pressure. | R506,R511 | All-arena centre admission, caller-anchor rejection, count/HP/ammo/lifetime/ingress/separation/route-diversity/nav plus player/bot/team/life target matrix and fixed-start/cover/seed/sample survival-percentile stress. | Swarm eligibility and pressure evidence. |
| K10 | Possession owner | K08,F08,P04[DEC-06=FROZEN,DEC-13=FROZEN] | Deterministic valid centre-map spawn; non-inverted keyboard/mouse/gamepad yaw, pitch, thrust, strafe and vertical input; autonomous speed exactly 2× manual; clean mode transitions; 30s fuel, 2×20, immobile fully vulnerable body and exactly-once clean exits. | R507,R510 | All-arena centre admission, signed full-input matrix, exact 20m/s-vs-10m/s comparator, transition reset, OOB/body death/drone death/fuel/ammo/disconnect/round-end matrix. | Possession evidence. |
