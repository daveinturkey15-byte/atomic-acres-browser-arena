...
| K04 | RNG/Nuke owner | F12,K03,P04[DEC-03=FROZEN,DEC-13=FROZEN] | Derive an exact integer-unit cost-monotonic pool from the unique typed catalog: every non-retired streak except care package appears once with positive weight, retired/care-package entries have zero weight, the roll stays hidden, and verifier-green care-only Nuke is exactly 1%. | R503,R512,R604 | Availability-enum/catalog uniqueness, derived-set equality, forced-every-reward, privacy, integer arithmetic/property, repeat-seed and forced-Nuke tests. | Stored catalog/pool-set equality, roll/weight/completeness and Nuke report. |
