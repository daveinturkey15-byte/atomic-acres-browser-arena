...
CLAIMED original owner-request relay, 2026-09-09: ordinary damaging combat explosions leave residual smoke with half the CURRENT SMOKE GRENADE ground area. Later root overnight contract explicitly clarifies half the ORIGINAL4.2m grenade area, not half the enlarged HF-563 area; this later clarification governs the bounded implementation below. Reserved alongside HF-563; resource intake73 is a reference ID, not this feedback ID.
...
### HF-563/HF-564 bounded source candidate — 2026-09-09 overnight

VERIFIED local runtime-impact implementation in isolated `contrib/dave-gaming-pc/codex/overnight-smoke-20260909`, based on combined visual/MP staging `f83acdd80542b8eac834890782113ce52997e0e6`: main smoke radius `4.2 * Math.sqrt(3)` and frag/Semtex residual radius `4.2 / Math.sqrt(2)`. These are nominal circular-footprint area ratios3 and0.5, not a measured visible ground-pixel area. Existing deterministic5–10s lifetime, palette,12-volume/8-corridor limits,0.42m/900ms corridors, schema2 and48KiB wire limit are unchanged. Prewarm uses the canonical main radius without increasing pool/card budgets.

VERIFIED authority boundary: an exhaustive grenade policy creates residue only for frag/Semtex, one main cloud for smoke, none for flash. Actual `explodeGrenade` calls guarded host/offline registration before its remote-owner damage return, so admitted guest grenades are included while clients cannot author volumes. Existing owner/action-nonce identity suppresses active duplicates; this is not a new lifetime-wide replay guarantee. The entire explosion damage/visual tail below `audio.explosion(afterPresentationDetach)` is byte-identical to the base. Generic visual callbacks and support/bolt/nuke families remain untouched, avoiding replica or recursive smoke producers.

VERIFIED focused evidence: unchanged runtime with unused policy scaffolding failed6 of18 new tests; implemented candidate passes67/67 across11 affected files plus TypeScript checking. Actual-source detonation/spawn tests execute production functions with real authority/math and spied transport/presentation; wire/late-join/expiry/reset, bot LOS/shot corridors, volume/corridor/wire caps and existing presentation/disposal tests pass. One additional unchanged-damage digest assertion was added after the RED run. External receipts: `can-you-hear-me-ok-hello/outputs/aa-visual-run/smoke-contract-{red,green}.json`; source inventory in `SMOKE-SOURCE-AUDIT.md` alongside them. These are local deterministic checks, not live multiplayer or screenshot evidence.

CLAIMED independent root review of `4b00b7b9418918a6fcecf8bc181dec3329f94388`: full bounded diff/authority review found no blocking source defect;19/19 new tests passed independently. VERIFIED local immutable build:630files, tree `f20ca2d510b7d92d82f894fd4a1513436228ca806f7cbfb503c473f68d2cf126`. Runtime acceptance remains OPEN.

VERIFIED saturation policy is unchanged oldest-first eviction from a shared12-volume pool, now including residue. A frag/Semtex burst can evict a still-active full grenade cloud before its nominal5–10s expiry; the lifetime is not a guaranteed minimum residency. Explicit source regression covers one full cloud followed by12 residual clouds and matching replica snapshot. OPEN burst multiplayer acceptance must verify host/guest visual removal, bot LOS, replication and performance under this eviction pressure. Do not raise the cap or promise every cloud its full nominal lifetime without separate evidence and approval.

VERIFIED continuation10September05:57BST: saturation followup and actual-source detonation tests20/20PASS, TypeScript checking PASS; receipt `smoke-burst-green.json`. Initial added-test failure was an incorrect insertion-order expectation, corrected to the production canonical-ID ordering after source inspection; it is not a product regression. No production behavior changed in this followup.

OPEN acceptance: real host/guests, grenade trajectories, visible footprint/opacity, first impact, expiry/reentry, bot/human perception, mobile/night/quality profiles, concurrent overdraw/FPS and canonical soak. The browser launch policy denial remains unresolved and is not bypassed. No promotion or known-good replacement; broader ordinary-explosion coverage (crossbow/support and incomplete rocket/mine inventory) remains OPEN, not silently included. Earlier implementation prerequisites above remain acceptance gates, not waived by these tests.

## HF-565 — Browser-only Options; native runtime deferred

CLAIMED direct owner request relayed by root on2026-09-09: remove the RTX/native-runtime option and explainer, focus on browser WebGPU graphics, and defer native Vulkan indefinitely. Reserved after a fresh no-match collision check. This supersedes the earlier HF-418 explainer requirement, not the actual graphics profiles.

VERIFIED local candidate2026-09-09: exact menu ladder PERFORMANCE, BALANCED, QUALITY, MAX, CUSTOM; no native option, dialog, binder or retained dialog CSS. Graphics preset values and implemented software ray-traced reflection settings remain unchanged; old stored `rtx-native-runtime-info` values still normalize safely. Focused shell/profile/settings tests84/84 pass across4files with unchanged pinned control-set hashes; TypeScript and Vite build pass. Retargeted `scripts/qa/verify-rtx-explainer-headless.mjs` booted actual Chrome WebGPU Options on41962: five actual selection/staging checks preserve persisted graphics before Apply,42advanced controls remain, zero native nodes, zero page/device/uncaptured errors. Receipt: `C:/Users/david/Documents/Codex/2026-09-09/can-you-hear-me-ok-hello/outputs/aa-visual-run/hf565-options/rtx-explainer-receipt.json`; screenshot `browser-only-options.png` alongside it, independently inspected by root and worker. Captured bundle `legacy-main-Ck4L6aUz.js`. Historical release notes remain historical; RTX5080 benchmark-machine references are not native-runtime advertising.

OPEN publication: local candidate only, no public deployment or promotion. Native Vulkan remains parked indefinitely; browser visual/performance and unrelated provenance failures remain separate open gates.
