...
| **R-5** | Reload is invisible to other players | always | `PlayerSnapshot` has no reload field (`protocol.ts:142-162`) | not replicated | **FIXED — MEASURED** — `reloading` is replicated in `PlayerSnapshot` and the reload row measure is PASS | ORIGINAL |
...
| Reproduced by the driver this run (MEASURED) | **11 row IDs** in the final complete artifact (L-1, L-4, L-7, L-9, X-2, P-3, P-4, P-5, R-1, R-2, R-5) |
