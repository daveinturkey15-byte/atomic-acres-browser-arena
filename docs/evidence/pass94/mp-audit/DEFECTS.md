...
| **P-2** | Rejected claim rolls back up to 1.5 s later with no signal | drop the `pickup-result` | `expirePendingLocalPickup`, deadline 1500 ms | `legacy-main.ts:15567-15577` | **FIXED / TRACED** — pending auto-scavenge now records a rollback and timeout emits `PICKUP TIMED OUT`; direct timeout scenario remains OPEN | no — 2026-08-22 |
| **P-3** | Guest B re-simulates guest A's **unvalidated** pickup claim and never gets a correction | 3 peers only | `relay` in `baseline-audit.json`: `pickup` is in guestA's `sent` **and** guestB's received set | `network.ts:1285-1286` — `pickup` is not in the allow-list at `:1271-1280`, so the transport blindly relays a guest's raw claim to every other guest; `pickup-result` is unicast (`:15486`), so guest B has no repair channel | **FIXED — MEASURED** — guests do not relay raw pickup claims; host validates and broadcasts the canonical result/correction. The three-peer driver row measure is PASS | no — relay 2026-07-21, handler ORIGINAL |
| **P-4** | A guest that rejects a pickup freezes that player until they die | follows P-3 | — | `legacy-main.ts:13498-13517` — the `return` at `:13516` runs on **guests** too, so guest B drops every later state message from that player | **FIXED — MEASURED** — rejected claims are not relayed and the correction is broadcast to every peer; the three-peer driver row measure is PASS | ORIGINAL (guard), 2026-08-22 (the `return`) |
| **P-5** | Auto-scavenge mutates guest state with no rollback record | walk over a drop | — | `legacy-main.ts:15375-15416` sets no `pendingLocalPickup`; the result handler's `pending === null` branch then force-switches the weapon (`:15558-15560`) | **FIXED — MEASURED** — auto-scavenge records prior inventory/drop state and the three-peer auto-scavenge scenario reports PASS | no — 2026-07-25 / 2026-08-22 |
| **P-6** | Drops are not host-authoritative: every peer spawns its own copy | any death | ids match (`death-${nonce}`), **content does not** | `legacy-main.ts:15091` called from `processDeath` on all peers; weapon/position from each peer's local view | **TRACED; MEASURED OPEN** — host canonical `DeathMessage.drop` is now created and broadcast before processing; the final real-death pickup scenario still rejected the staged claim, so end-to-end closure is not claimed | ORIGINAL |
...
| **P-8** | With the corrected staging: the guest picks the gun up, then it **silently reverts** | `after2` run, both guests | `staged: smg`, `holding: carbine`, `interact.returned: **true**`, `sentPickup: **true**`, `gotPickupResult: **true**`, `weaponAfter: carbine` — trace `out:pickup, in:pickup-result` | the host **rejected** the claim. But the drop was staged by a guest-local QA hook, so the host never observed the death and legitimately has no such drop: `unknown-drop` is *correct* host behaviour here | **TRACED; MEASURED OPEN** — the canonical host-death path is present, but the complete driver still records a rejected attempted claim and does not prove successful real-death pickup convergence | n/a |
...
| **R-2** | Same, via a lag spike instead of a death | bounded movement resync | `reload-result` stamped with the host's `lifeId` is dropped by the guest's own filter (`:6471`) | `legacy-main.ts:13559` bumps the host's lifeId on `movement.resynchronized` with **no** corresponding guest increment; every rejection is then undeliverable | **FIXED — MEASURED** — continuity is preserved across movement resync and the post-respawn reload row measure is PASS | ORIGINAL / 2026-08-02 |
| **R-3** | A legitimate in-flight reload is cancelled by a shot the host is about to reject | fire while reloading, out of ammo | guest gets `status:'cancelled'`, `reloadState` nulled silently | `legacy-main.ts:14341` — `cancelRemoteReloadAuthority` runs **before** the `missing-history` / `bad-origin` / `empty-magazine` rejections return | **FIXED / TRACED** — remote reload cancellation now occurs only after shot admission; direct rejection scenario remains OPEN | no — 2026-08-02 |
| **R-4** | Ammo desync never self-heals | any missed projection | — | the host's only push-repair channel is inert: a `state` message about the guest applies **only `hp`** and returns (`:13308-13320`), never reading the attached `combatInventory` the host sent at `:13513` | **FIXED / TRACED** — authoritative self-state repair now applies combat inventory projection; direct missed-projection scenario remains OPEN | no — 2026-08-02 / 2026-08-22 |
| **R-5** | Reload is invisible to other players | always | `PlayerSnapshot` has no reload field (`protocol.ts:142-162`) | not replicated | **FIXED / TRACED — RUNTIME VISIBILITY OPEN** — `reloading` is replicated and exposed in the audit view; R-5 now requires remote visibility, intent/result, ammo growth, and host ammo agreement. The historical artifact recorded `otherSeesReloading:null` and four `RELOAD-NOT-VISIBLE` findings. | ORIGINAL |
...
| ID | Symptom | Repro | Trace / evidence | Root cause | Fix | Months-old |
|---|---|---|---|---|---|---|
| **L-1** | **START enabled while the roster was incomplete** | host alone, before a guest connects | **MEASURED** — `LOBBY-START-EARLY` fired during both joins | `private-match.ts:392-400` `canHostCommitStart` requires only `connected.length >= 1` | **FIXED — MEASURED** — host-alone start is disabled and the final lobby row measure is PASS | no — 2026-07-25 |
| **L-2** | Players inside the 90 s rejoin grace are invisible to the start gate | start while someone is rejoining | — | `private-match.ts:399` filters to `connected`, which `markLobbyDisconnected` has already set false | **FIXED / TRACED** — disconnected reservations now block host start; direct grace-period scenario remains OPEN | no — 2026-07-25 |
| **L-3** | Host renders itself `SETTING UP` next to an **enabled** START | always | — | two divergent predicates: `canHostStart` (host must be ready) vs `canHostCommitStart` (host exempt); the UI uses the exempting one and the commit path force-writes host ready | **FIXED / TRACED** — UI and commit use the same host-ready predicate; focused test and driver assertion are present, final runtime recheck remains OPEN | no — 2026-07-21 |
| **L-4** | Guest and host see different lobbies | first seconds after JOIN | 10 surfaces enumerated in REPORT.md §Lobby | `renderPrivateLobby` falls back to local state: roster, every config field, ready label, ping, HOST badge; `#join` reveals the panel **without** calling `renderPrivateLobby()`, so a joining guest sees fabricated `1 / 4` markup with an enabled READY that dead-ends | **FIXED / TRACED; NARROW MEASURE** — lobby rendering now consumes only the authoritative snapshot; the driver measured agreement on phase, revision, arena, and members, not all ten DOM surfaces | **ORIGINAL** |
...
| **L-7** | The displayed countdown is wrong by the raw host↔guest OS clock offset | any start | clamp `max(1, min(5, …))` hides it: the guest pins at `DEPLOYING IN 5` | `:10844` subtracts the guest's `Date.now()` from the **host's** epoch; `estimateHostClockOffset` exists and **has no caller outside tests**. (The actual deploy instant *is* correctly skew-mapped) | **FIXED — MEASURED** — countdown uses the host-clock mapping; all three peers reported `DEPLOYING IN 5` in the final driver artifact | no — 2026-09-01 |
| **L-8** | A promoted successor host is silently ignored by its own followers | host crash | successor restarts revision below its followers' | `:8855` restores `revision` from the mirror, then broadcasts `+1`; guests hold a higher revision and drop every snapshot at the `<` guard (`:10450`). No repair path | **FIXED / TRACED; MEASURED OPEN** — successor snapshots are accepted when authority changes even if revision is lower; crash-promotion scenario remains OPEN | **ORIGINAL** |
| **L-9** | The roster is destroyed and rebuilt at least every 2 s on both sides | idle in the lobby | `clock-ping` (2 s/guest) drives a full `broadcastHostLobby` + `roster.innerHTML` rewrite | `:10769` + `:9971` — `revision` becomes latency telemetry rather than a state-change signal, which is the root enabler of L-8; an open DHV dropdown cannot survive | **FIXED — MEASURED** — clock telemetry no longer bumps lobby revision or rebuilds the roster; final artifact measured revision 10 -> 10 | **ORIGINAL** |
| **L-10** | A countdown joiner gets a start deadline already in the past | join at t+4.5 s | fixed 5 s lead stamped at host-click time | `:9671-9676` | **FIXED / TRACED; MEASURED OPEN** — admitted joiners receive the current authoritative phase/deadline; late-join scenario remains OPEN | no — 2026-08-02 |
| **L-11** | A rejoin that begins in `countdown` and lands after `active` regresses **every** guest's phase | race | `currentPhase` captured before two `await`s, then published at a higher revision | `:9563` + `:9668` | **FIXED / TRACED; MEASURED OPEN** — phase is committed after the asynchronous admission decision; countdown-to-active race scenario remains OPEN | no — 2026-08-02 |
| **L-12** | `lobby-start` accepted at equal revision with no nonce dedupe | countdown joiner gets both the broadcast and the unicast | `>=` at `:10706`, no `processedNonces` consult | absorbed only by luck of the admission identity dedupe | **FIXED / TRACED; MEASURED OPEN** — accepted start nonces are deduplicated; duplicate-delivery scenario remains OPEN | **ORIGINAL** |
...
| | Count |
|---|---|
| Rows total | **43 row IDs verified** (the historical header said 42; no row was deleted) |
| **Fixed this lane** | **24 source/traced or measured** (P-2, P-3, P-4, P-5, R-2, R-3, R-4, R-5, L-1, L-2, L-3, L-4, L-7, L-8, L-9, L-10, L-11, L-12, X-2 plus prior fixes P-1, R-1, W-1, L-5, L-6) |
| TODO / OPEN | **17** (including P-6/P-8, direct scenario gaps, and untouched groups) |
| Other lane | 2 (R-6, X-3) |
| ORIGINAL (survived since 2026-07-25, never corrected) | 12 |
| Reproduced by the driver this run (MEASURED) | **10 row IDs** in the final complete artifact (L-1, L-4, L-7, L-9, X-2, P-3, P-4, P-5, R-1, R-2); R-5 is excluded because visibility was missing |
