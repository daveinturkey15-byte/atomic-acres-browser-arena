...
- **Symptom:** native Python resolves Git Bash `$PWD` as `C:\c\...`. **Cause:** MSYS-style `/c/...` paths were passed to a native process. **Correction:** the runner normalizes drive-prefixed MSYS arguments before resolving them. **Verify:** the documented `--repo "$PWD"` smoke invocation reaches `git rev-parse` in the intended worktree.
