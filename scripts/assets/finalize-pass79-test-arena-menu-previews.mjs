/**
 * finalize-pass79-test-arena-menu-previews.mjs — owner 2026-08-30.
 *
 * Test1 and Test2 shipped menu previews that were BYTE-IDENTICAL copies of
 * other arenas: test1.{mp4,webm,webp} were gun-range.*, test2.* were
 * high-seas.*. Hovering Test1 in the live menu played the Gun Range flyover and
 * hovering Test2 played the superyacht. They were placeholder stubs dropped in
 * on 2026-08-30 and never replaced.
 *
 * This finalizer is the third additive family, built to the pattern
 * finalize-pass77-arena-menu-previews.mjs established and for the same reason:
 * the retained Pass 66 four-arena family is bound byte for byte to a cache-family
 * lock, and reopening it would mean re-rendering four owner-approved previews to
 * add two new ones. So Test1/Test2 get their own family beside it, using the
 * same staged frames, the SAME encoding profiles read out of the retained
 * masters choreography, the same budgets and the same runtime file layout.
 *
 ...
 * shipped six arenas were encoded with, so the two new files are members of the
 ...
 *
 *   node scripts/assets/finalize-pass79-test-arena-menu-previews.mjs
 *
 * AA_PASS79_RECEIPT overrides the capture receipt path.
 */
...
const reviewRoot = path.join(root, 'docs/assets/pass79-menu-previews');
const provenanceRoot = path.join(root, 'source-assets/menu/pass79-test-arena-previews');
 ...
 * that slot belongs to the retained Pass 66 family: verify-pass65-menu-preview-production.mjs
 * pins its digest AND asserts its arena roster is exactly the retained four.
 * A Test1/Test2 capture written there would take the retained gate red, so this
 * capture is run with AA_PREVIEW_REVIEW_ONLY=1 and its receipt lands on the
 * review path instead. That is a receipt-ROUTING constraint, not a quality one:
 * the capture itself ran the canonical recipe on the canonical WebGPU backend at
 * the canonical 2560x1440, which is re-derived below rather than taken on trust.
 */
const RECEIPT_PATH = path.join(root, process.env.AA_PASS79_RECEIPT
...
/** Same quiet beds as the retained and pass77 families so no card stands out. */
...
function assertAdditive() {
  ...
  if (Object.keys(EXTENSION.arenas ?? {}).join(',') !== ARENAS.join(',')) {
    throw new Error(`pass79 extension choreography must describe exactly ${ARENAS.join(',')} in that order`);
  }
  if (EXTENSION.inheritsMotionContract !== MASTERS.recipeId) {
    throw new Error('pass79 extension recipe does not inherit the Pass 66 motion contract');
  }
...
async function verifiedCapture(receipt, arena) {
  ...
  // WebGPU is the canonical backend for these two arenas and both are known-good
  // on it. A compat capture is not silently accepted here: it must name itself.
...
function assertEncodedContract(arena, stagingRoot) {
  ...
  if (failures.length > 0) throw new Error(`Pass 79 encoded contract failed:\n- ${failures.join('\n- ')}`);
}

/**
 * The defect this family exists to fix, asserted on the bytes it just wrote.
...
async function assertDistinctFromEveryOtherArena(installed) {
  const others = [...RETAINED_ARENAS, ...PASS77_ARENAS];
  ...
  for (const record of installed) {
    ...
    if (owner) throw new Error(`${record.path} is byte-identical to ${owner}; that is the placeholder defect this family exists to remove`);
...
async function main() {
  ...
  const staging = await mkdtemp(path.join(os.tmpdir(), 'aa-pass79-menu-'));
  ...
  for (const arena of ARENAS) {
    ...
    files.push({
      ...
      capture: {
        ...
        // worktree, so this is routinely false and must not be hidden.
  ...
  const provenance = {
    ...
    note: 'Additive family for Test1/Test2. Replaces the 2026-08-30 placeholder stubs, which were byte-identical copies of the gun-range and high-seas media. The retained pass66-runtime-preview-v15 media, its cache-family lock and the pass77 family are untouched by this finalizer.',
  ...
  const entry = {
    ...
    modifications: `Captured offline from the actual authoritative Test1 and Test2 runtime arenas on the canonical WebGPU route with the deterministic Pass 66 camera recipe, visual time and baked cockpit overlay, then encoded with the Pass 66 profiles and budgets. These files replace placeholder stubs that were byte-identical copies of the gun-range and high-seas previews. No downloaded or sampled art is used.`,
