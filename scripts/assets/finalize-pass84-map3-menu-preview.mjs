/**
 * finalize-pass84-map3-menu-preview.mjs — MAP3, owner 2026-09-02 (HF-405).
 *
 * Map 3 was registered as a selectable arena in 45f45cc2 with NO menu preview
 * media, so its card rendered the labelled PREVIEW STANDBY placeholder and
 * `npm run qa:pass77:menu-previews` was correctly red on four counts. This
 * finalizer closes that: it encodes Map 3's own flyover from Map 3's own
 * authoritative runtime capture.
 *
 * It is the FOURTH additive family, built to the pattern
 * finalize-pass77-arena-menu-previews.mjs established and
 * finalize-pass79-test-arena-menu-previews.mjs repeated, for the same reason
 ...
 * owner-approved previews to add one new one. So Map 3 gets its own family
 ...
 * dropped in instead. Map 3 was added to that merge in the same commit as this
 * file, and `assertDistinctFromEveryOtherArena` below asserts the defect is
 * absent on the exact bytes this script just wrote.
 *
 ...
 *
 *   AA_PREVIEW_REVIEW_ONLY=1 AA_PREVIEW_ARENAS=map3 npx tsx scripts/assets/generate-pass65-runtime-menu-previews.ts
 *   node scripts/assets/finalize-pass84-map3-menu-preview.mjs
 *
 * AA_PASS84_RECEIPT overrides the capture receipt path.
 */
...
const reviewRoot = path.join(root, 'docs/assets/pass84-menu-previews');
const provenanceRoot = path.join(root, 'source-assets/menu/pass84-map3-preview');
 ...
 */
const RECEIPT_PATH = path.join(root, process.env.AA_PASS84_RECEIPT
...
function assertAdditive() {
  ...
  if (Object.keys(EXTENSION.arenas ?? {}).join(',') !== ARENAS.join(',')) {
    throw new Error(`pass84 extension choreography must describe exactly ${ARENAS.join(',')} in that order`);
  }
  if (EXTENSION.inheritsMotionContract !== MASTERS.recipeId) {
    throw new Error('pass84 extension recipe does not inherit the Pass 66 motion contract');
  }
  for (const arena of ARENAS) {
    if (RETAINED_ARENAS.includes(arena) || PASS77_ARENAS.includes(arena) || PASS79_ARENAS.includes(arena)) {
...
async function verifiedCapture(receipt, arena) {
  ...
  // WebGPU is the canonical backend and Map 3 is known-good on it (it is a
  // WebGPU-only TSL arena). A compat capture is not silently accepted here: it
  // must name itself.
...
async function assertDistinctFromEveryOtherArena(installed) {
  const others = [...RETAINED_ARENAS, ...PASS77_ARENAS, ...PASS79_ARENAS];
...
async function main() {
  ...
  const staging = await mkdtemp(path.join(os.tmpdir(), 'aa-pass84-menu-'));
  ...
  const provenance = {
    ...
    note: 'Additive family for Map 3 (HF-405), the arena registered on 2026-09-02. Replaces the labelled PREVIEW STANDBY card it shipped with for one commit - never a byte-copy of another arena, which is the placeholder defect the pass79 family had to undo. The retained pass66-runtime-preview-v15 media, its cache-family lock, and the pass77 and pass79 families are untouched by this finalizer.',
  ...
  const entry = {
    ...
    modifications: 'Captured offline from the actual authoritative Map 3 runtime arena on the canonical WebGPU route with the deterministic Pass 66 camera recipe, visual time and baked cockpit overlay, then encoded with the Pass 66 profiles and budgets. No downloaded or sampled art is used.',
