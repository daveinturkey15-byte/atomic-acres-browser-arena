/**
 * finalize-pass85-nuketown2-menu-preview.mjs - NUKETOWN2, owner 2026-09-02 (HF-407).
 *
 * The Nuke Town Rebuild was registered as a selectable, multiplayer arena on the
 * day it was authored with NO menu preview media, so its card rendered the
 * labelled PREVIEW STANDBY placeholder through `MEDIA_PENDING_ARENAS` - a list
 * whose own comment says it should stay empty, and a state AGENTS.md' arena
 * preview rule fails. This finalizer closes that: it encodes the rebuild's own
 * flyover from the rebuild's own authoritative runtime capture.
 *
 * It is the FIFTH additive family, built to the pattern
 * finalize-pass77-arena-menu-previews.mjs established and
 * finalize-pass79-test-arena-menu-previews.mjs and
 * finalize-pass84-map3-menu-preview.mjs repeated, for the same reason all three
 * exist: the retained Pass 66 four-arena family is bound byte for byte to a
 * cache-family lock, and reopening it would mean re-rendering four
 * owner-approved previews to add one new one. So the rebuild gets its own family
 * beside them, using the same staged frames, the SAME encoding profiles read out
 ...
 * shipped arenas were encoded with, so the new files are members of the same
 * shipped set rather than lookalikes of it.
 *
 ...
 * hovering Test1 in the live menu played the Gun Range flyover. The reason is
 * recorded in generate-pass65-runtime-menu-previews.ts: the two arenas reached
 * ARENA_SELECTIONS but never reached that tool's choreography merge, so its
 * roster assertion rejected EVERY capture and a placeholder was dropped in
 * instead. nuketown2 was added to that merge in the same commit as this file,
 * and `assertDistinctFromEveryOtherArena` below asserts the defect is absent on
 * the exact bytes this script just wrote.
 *
 ...
 *
 *   AA_PREVIEW_REVIEW_ONLY=1 AA_PREVIEW_ARENAS=nuketown2 npx tsx scripts/assets/generate-pass65-runtime-menu-previews.ts
 *   node scripts/assets/finalize-pass85-nuketown2-menu-preview.mjs
 *
 * AA_PASS85_RECEIPT overrides the capture receipt path.
 */
...
const reviewRoot = path.join(root, 'docs/assets/pass85-menu-previews');
const provenanceRoot = path.join(root, 'source-assets/menu/pass85-nuketown2-preview');
 ...
 * arena roster is exactly the retained four. A Map 3 capture written there would
 ...
 */
const RECEIPT_PATH = path.join(root, process.env.AA_PASS85_RECEIPT
...
function assertAdditive() {
  ...
  if (Object.keys(EXTENSION.arenas ?? {}).join(',') !== ARENAS.join(',')) {
    throw new Error(`pass85 extension choreography must describe exactly ${ARENAS.join(',')} in that order`);
  }
  if (EXTENSION.inheritsMotionContract !== MASTERS.recipeId) {
    throw new Error('pass85 extension recipe does not inherit the Pass 66 motion contract');
  }
  for (const arena of ARENAS) {
    if (RETAINED_ARENAS.includes(arena) || PASS77_ARENAS.includes(arena) || PASS79_ARENAS.includes(arena) || PASS84_ARENAS.includes(arena)) {
...
async function verifiedCapture(receipt, arena) {
  ...
  // WebGPU is the canonical backend and the rebuild is known-good on it (an
  // all-code WebGPU/TSL arena whose boot smoke passes headless on native
  // WebGPU). A compat capture is not silently accepted here: it must name itself.
...
function assertEncodedContract(arena, stagingRoot) {
  ...
  if (failures.length > 0) throw new Error(`Pass 85 encoded contract failed:\n- ${failures.join('\n- ')}`);
}
...
async function main() {
  ...
  const staging = await mkdtemp(path.join(os.tmpdir(), 'aa-pass85-menu-'));
  ...
  const entry = {
    ...
    modifications: 'Captured offline from the actual authoritative Nuke Town Rebuild runtime arena on the canonical WebGPU route with the deterministic Pass 66 camera recipe, visual time and baked cockpit overlay, then encoded with the Pass 66 profiles and budgets. No downloaded or sampled art is used.',
