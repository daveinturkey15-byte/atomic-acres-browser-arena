"""finalize_pass87_raid2_loading_backdrop.py - RAID2, owner 2026-09-02 (HF-408).

Raid Rebuild was registered as a selectable arena on 2026-09-02 and had no
DEPLOYMENT LOADING BACKDROP, so `npm run qa:pass77:menu-previews` reported
"selectable arena raid2 ships no deployment loading backdrop" - a real gap this
lane left, found by running that verifier in the repair pass. This script
derives the backdrop from raid2's OWN authoritative WebGPU runtime capture -
...
gun-range-loading.webp and Test2 shipped high-seas-loading.webp. Map 3 shipped
its own on 2026-09-02 by the same route; this is the same script for raid2.
...
the opposite, and raid2 does the same thing for the same reason: at the
family's own ceiling quality of 88 this content encodes far UNDER the band's
lower bound. That is a property of the content, not of the encode - a bleached
travertine estate under a high sun is large flat paving and short soft shadows,
i.e. very little high-frequency detail, and WebP spends bytes on exactly that.
Inflating the file to reach the band would mean encoding at a quality no other
backdrop uses, purely to hit a number.
...
arena's backdrop - before they are allowed to stay on the public shelf.
    ...
    python scripts/assets/finalize_pass87_raid2_loading_backdrop.py

AA_PASS87_LOADING_RECEIPT overrides the capture receipt path.
"""
...
PROVENANCE_ROOT = os.path.join(ROOT, "source-assets", "loading", "pass87-raid2-loading")
MANIFEST_PATH = os.path.join(ROOT, "assets.manifest.json")
RECEIPT_PATH = os.path.join(
    ROOT,
    os.environ.get(
        "AA_PASS87_LOADING_RECEIPT",
        "artifacts/pass65/menu-preview-rotor-review/runtime-capture-receipt.json",
    ),
)

FAMILY_ID = "pass87-raid2-loading-backdrop"
...
# See the module docstring: the same floor map3 set, measured here on raid2.
...
LOADING_FRAME = int(os.environ.get("AA_PASS87_LOADING_FRAME", "180"))
REVIEW_NOTE = (
    "Chosen by eye from the fresh capture, over the poster frame (60) and over 30/120/210. The "
    "estate reads as one architectural mass with the pool terrace running away to the left, the "
    "open-to-sky courtyard legible inside the house, the circular drive and both balconies in "
    "frame, and the sport court and garage wing anchoring the two ends. It is the frame that "
    "sells what this rebuild actually changed - three lanes and one building, not a warren."
)
...
def main() -> None:
    ...
    provenance = {
        ...
        "note": (
            "Deployment loading backdrop for Raid Rebuild (raid2, HF-408), derived from the arena's own "
            "authoritative WebGPU runtime capture. raid2 never shipped a placeholder: this is its first "
            "backdrop. The six Lane I backdrops, the pass79 pair, the pass84 map3 backdrop and all "
        ...
        "encode": {
            ...
            "lowerByteBoundSubstitution": (
                ...
                "ceiling quality of 88 this content encodes well under it because a bleached travertine "
                "estate under a high sun carries very little high-frequency detail, and raising quality above the "
    ...
    entry = {
        ...
        "source": "Offline WebGPU capture of the Raid Rebuild authoritative runtime arena",
        ...
        "modifications": (
            "Captured offline at native 1440p from the actual authoritative Raid Rebuild production WebGPU arena "
