...
from pass83_teams import TEAMS, SKILLS, SKILL_ROOT  # noqa: E402
...
def build_spec(team_name, round_no, tasks):
    ...
    coord = (
        ...
        "There are only 2 slots machine-wide, shared with every other harness.\n"
        "PREFER HEADLESS: installed Chrome with channel:'chrome', headless:true gets a real "
        "hardware WebGPU device and needs NO slot at all. Keep the headed slots for work that "
        "truly needs a window, such as two-window multiplayer. Two gotchas: navigator.gpu "
        "requires a SECURE CONTEXT so navigate to 127.0.0.1 before probing, never about:blank; "
        "and an adapter is not a device - call requestDevice() and check adapter.info.vendor, "
        "because a Microsoft vendor string means the software rasteriser.\n\n"
        "VERIFICATION IS THE EXPENSIVE PART, NOT YOU. Measured on this machine 2026-08-25: an "
        "agent costs about 0.35 GB, but `npx vitest run` peaks at 5.63 GB and `npx tsc "
        "--noEmit` at 3.41 GB - SIXTEEN TIMES the agent that runs it. Five agents verifying at "
        "once is roughly 28 GB, the whole machine. What looked like too many agents was really "
        "too many concurrent test runs. Therefore:\n"
        "  - Run `npx vitest run <your own test files>`, NOT the whole suite. The gauntlet runs "
        "the full suite once per round on your behalf.\n"
        "  - Before any FULL-suite or tsc run, take a verify slot:\n"
        f"      python {GOVERNOR} verify-acquire --id <your-task-id>\n"
        f"      python {GOVERNOR} verify-release --id <your-task-id>\n"
        "    Two slots machine-wide; release in a finally so a crash still frees one.\n"
        "Do not "
