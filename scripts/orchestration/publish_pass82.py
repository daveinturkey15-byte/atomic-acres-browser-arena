#!/usr/bin/env python3
"""Publish the Pass 82 candidate as its own gh-pages channel.

Sibling of publish_pass81.py. Adds `channels/pass82` beside the existing channels rather
...
DIST = os.path.join(SRC, "dist-pass82")
WORKTREE = os.path.join(SRC, ".gh-pages-publish")
CHANNEL = "channels/pass82"
...
# Nothing retired this pass. PASS 81 stays published and offered as the previous
# version (owner, 2026-08-30: keep the previous 1/2 versions visible); PASS 80 was
# already retired by the pass81 publish and stays retired.
RETIRE = {}

# Predecessor copy, restated for the pass82 chooser: PASS 81 is now the previous
# version; PASS 73 and PASS 72 remain retained comparison builds. Same rule as the
# pass81 publish: copy that names a card's role, never a bare pass number.
PREDECESSOR_COPY = {
    "pass81": {
        "label": "PASS 81 · PREVIOUS VERSION",
        "description": (
            "The build that was live before this one. Pick this if the newest "
            "pass misbehaves."
        ),
    },
    "experimental": {
...
DESCRIPTION = (
    "Pass 82: the cross-browser freeze is fixed - dying no longer forced every "
    "material in the map to rebuild its WebGPU pipelines (measured: Edge 21.8% "
    "of the window frozen, Firefox 11.5%, Chrome 8.5%, all now ~0%). If this "
    "browser's WebGPU compiler fails, the game retries and then switches itself "
    "to the compatibility renderer. Farcrysis hidden until ready."
)

COMMIT_MESSAGE = """publish: PASS 82 - cross-browser freeze removal (light-set stability)
...
def assert_build_is_not_stale():
    ...
    for root, dirs, files in os.walk(SRC):
        dirs[:] = [d for d in dirs if d not in {
            "node_modules", ".git", "dist", "dist-pass82", ".gh-pages-publish",
...
def main():
    ...
    try:
        assert_predecessors_offered({
            "pass82": {"pass": "PASS 82", "path": "channels/pass82"},
