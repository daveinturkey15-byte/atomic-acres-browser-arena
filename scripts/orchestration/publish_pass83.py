#!/usr/bin/env python3
"""Publish the Pass 83 candidate as its own gh-pages channel.

Sibling of publish_pass82.py. Adds `channels/pass83` beside the existing channels rather
...
DIST = os.path.join(SRC, "dist-pass83")
WORKTREE = os.path.join(SRC, ".gh-pages-publish")
CHANNEL = "channels/pass83"
...
KEEP_AT_LEAST = {"experimental", "previous", "pass82"}

# Nothing retired this pass. PASS 82 stays published and offered as the previous
# version; PASS 81 and older retained trees stay exactly where earlier publishes
# pinned them.
RETIRE = {}

# Predecessor copy, restated for the pass83 chooser: PASS 82 is the previous
# version; PASS 81/73/72 remain retained comparison builds. Same rule as every
# earlier publish: copy that names a card's role, never a bare pass number.
PREDECESSOR_COPY = {
    "pass82": {
        "label": "PASS 82 · PREVIOUS VERSION",
        "description": (
            "The build that was live before this one - the cross-browser freeze "
            "fix. Pick this if the newest pass misbehaves."
        ),
    },
    "pass81": {
        "label": "PASS 81 · RETAINED",
        "description": (
            "The retained pre-freeze-fix build, kept for comparison."
        ),
    },
    "experimental": {
        "label": "PASS 73 · RETAINED",
        "description": (
            "Approved Pass 73 first-person, gameplay, world-integrity and "
            "renderer-correction work, retained for comparison and rollback."
        ),
    },
    "previous": {
        "label": "PASS 72 · RETAINED",
        "description": (
            "The exact Pass 72 Pages runtime, retained byte-for-byte for "
            "comparison and immediate rollback testing."
        ),
    },
}

DESCRIPTION = (
    "Pass 83: freezes fixed cross-browser (see Pass 82 notes), plus multiplayer "
    "lobby work: the host's deploy countdown now numbers 5-4-3-2-1 from one shared "
    "clock, and joins from a different build are refused with a clear message "
    "instead of silently playing a different map. The header badge now names this "
    "build correctly. Farcrysis hidden until ready."
)

COMMIT_MESSAGE = """publish: PASS 83 - lobby build identity, 5-4-3-2-1 deploy countdown, correct badge

Owner, 2026-08-31: "it just freezes every few seconds in firefox, mega unstable!
same issue with edge, unplayable." Measured cause: hiding the first-person
viewmodel on death removed the two structural viewmodel lights from Three's
WebGPU light set, which changed the LightsNode cache key and invalidated every
material program in the scene at once - 251 render pipelines rebuilt inside a
75s combat window, 99.2% of them inside compositor stalls. The hidden state now
uses the same retained-structural-lights contract as fullscreen suppression:
uniform writes, never a light-set change.

Measured on the fixed build, 180s lanes: Chrome 8.49% -> 0% frozen, Edge
21.77% -> 0.13% (one 240ms presentation-backpressure event), Firefox 11.49% ->
0%. In-combat pipeline creations 251 -> 0, shader modules 268 -> 0.

tsc 0; full vitest floor green; pipeline probe and cross-engine stall artifacts
under artifacts/qa/ in the contribution worktree. Evidence:
docs/PASS82_CROSS_BROWSER_FREEZE_2026-09-01.md.

Co-Authored-By: Oh My Pi (OMP) <noreply@anthropic.com>"""
...
def assert_build_is_not_stale():
    ...
    for root, dirs, files in os.walk(SRC):
        dirs[:] = [d for d in dirs if d not in {
            "node_modules", ".git", "dist", "dist-pass83", ".gh-pages-publish",
...
def main():
    ...
    try:
        assert_predecessors_offered({
            "pass83": {"pass": "PASS 83", "path": "channels/pass83"},
    ...
    # Refresh predecessor copy so each card names its role for this chooser. Only
    # the strings move; the paths and pass codes are untouched, so no retained
    # tree is re-pointed.
