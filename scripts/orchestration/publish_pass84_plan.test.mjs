// Contract test for scripts/orchestration/publish_pass84.py --dry-run (HF-400).
//
...
// pass83, keep pass83, add pass84, offer exactly those two, and resolve the in-build
// fallback to channels/pass83 - all without writing a byte into the fake tree.
//
...
const script = join(repo, 'scripts', 'orchestration', 'publish_pass84.py');

// What origin/gh-pages carried at 718a5295 (2026-09-02, verified with git ls-tree).
const LIVE_TREES = ['pass72-retained', 'pass81', 'pass82', 'pass83', 'recent-stable', 'the-big-one'];
...
const LIVE_CONFIG = {
  ...
  pass83: { label: 'PASS 83', description: 'x', pass: 'PASS 83', path: 'channels/pass83' },
};
...
function python() {
  ...
  assert.fail('no python on PATH - the publish_pass84 contract cannot run, and an unrun contract is not a pass');
}

function fakeGhPages(trees = LIVE_TREES) {
  const dir = mkdtempSync(join(tmpdir(), 'aa-pass84-ghpages-'));
...
test('the plan deletes every tree except pass83, keeps pass83, adds pass84, and the post-state is exactly {pass83, pass84}', () => {
  ...
  try {
    ...
    assert.deepEqual(plan.treesToDelete, LIVE_TREES.filter((tree) => tree !== 'pass83'));
    assert.equal(plan.treesToDelete.length, 5);
    assert.deepEqual(plan.treesKept, ['pass83']);
    assert.equal(plan.treeAdded, 'pass84');
    assert.deepEqual(plan.postState, ['pass83', 'pass84']);
    assert.equal(plan.channel, 'channels/pass84');
    assert.equal(plan.backup, 'channels/pass83');
    ...
    assert.match(output, /would keep\s+\['channels\/pass83\/'\]/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('the chooser carries exactly two cards, keyed for the shell aliases: experimental -> PASS 84 live, previous -> PASS 83 backup', () => {
  const dir = fakeGhPages();
  try {
    ...
    assert.deepEqual(
      ...
      { pass: 'PASS 83', path: 'channels/pass83', label: 'PASS 83 · SAFE BACKUP' },
    );
    ...
    assert.deepEqual(plan.chooser.droppedLiveKeys.sort(), ['experimental', 'pass81', 'pass82', 'pass83', 'previous']);
...
test('the in-build fallback (src/bootstrap.ts via release-channels.json) resolves to channels/pass83', () => {
  // Red until src/bootstrap.ts prefers `pass83Backup`: the guard reads the real
  ...
  // offer a second card that 404s the moment the pass84 publish retires its tree.
  const dir = fakeGhPages();
  try {
    const { plan } = dryRun(dir);
    assert.equal(plan.fallback.path, 'channels/pass83',
      `in-build fallback resolves to '${plan.fallback.key}' -> ${plan.fallback.path}; HF-400 retires that tree. `
      + 'Re-pin `const stableFallback` in src/bootstrap.ts to releaseChannels.pass83Backup.');
    assert.equal(plan.fallback.pass, 'PASS 83');
...
test('refuses when the pass83 backup is not on gh-pages to be pinned, and plans no deletions', () => {
  const dir = fakeGhPages(LIVE_TREES.filter((tree) => tree !== 'pass83'));
  try {
    const { status, plan } = dryRun(dir);
    assert.equal(status, 2);
    assert.equal(plan.wouldPublish, false);
    assert.equal(plan.guards['backup-present'].ok, false);
    assert.match(plan.guards['backup-present'].detail, /no \['pass83'\] tree to pin as the safe backup/);
...
test('rollback re-points the default at PASS 83, keeps PASS 84 as the previous card, deletes nothing, needs no build', () => {
  const dir = fakeGhPages(['pass83', 'pass84']);
  try {
    const before = snapshot(dir);
    ...
    assert.equal('build-present' in plan.guards, false, 'a rollback must not depend on dist-pass84');
    assert.equal('build-freshness' in plan.guards, false, 'a rollback must not depend on dist-pass84');
    assert.deepEqual(plan.chooser.keys, ['experimental', 'previous']);
    assert.deepEqual(
      { pass: plan.chooser.channels.experimental.pass, path: plan.chooser.channels.experimental.path, deploymentState: plan.chooser.channels.experimental.deploymentState },
      { pass: 'PASS 83', path: 'channels/pass83', deploymentState: 'live' },
    );
    assert.deepEqual(
...
test('rollback refuses when gh-pages is not already exactly {pass83, pass84}', () => {
  const dir = fakeGhPages();
  try {
    const { status, plan } = dryRun(dir, '--rollback');
    assert.equal(status, 2);
    assert.equal(plan.guards['rollback-state'].ok, false);
    assert.match(plan.guards['rollback-state'].detail, /post-state is \[.*\], expected exactly \['pass83', 'pass84'\]/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
