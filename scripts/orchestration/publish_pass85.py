#!/usr/bin/env python3
"""Publish the Pass 85 candidate as its own gh-pages channel and retire every other tree.

Sibling of publish_pass84.py, with one policy change from the owner (HF-400).
...
So after this script runs, gh-pages carries EXACTLY two channel trees:
    ...
    channels/pass85   the live default (this cut)
    channels/pass84   the pinned safe backup (what was live when he said it)
...
and the post-state is asserted to be exactly {pass85, pass84} before anything is committed.
...
Guards kept from publish_pass84.py, unchanged in strictness: the build freshness guard
...
must resolve to the PASS 84 backup, not merely to something that exists.
...
DIST = os.path.join(SRC, "dist-pass85")
WORKTREE = os.path.join(SRC, ".gh-pages-publish")
CHANNEL = "channels/pass85"
LIVE_TREE = "pass85"
BACKUP_TREE = "pass84"
...
DESCRIPTION = (
    "Pass 85 lands the first of the 2026-09-02 evening list: Black Ops 2 style drop shots, the Firing Range roof netting that swallowed players, and first-person arms and knife that stay in frame."
)
...
# `previous` - which is exactly the alias the owner's policy needs: ?release=previous -> PASS 84.
BACKUP_COPY = {
    "label": "PASS 84 · SAFE BACKUP",
    "description": (
...
def build_channels(rollback=False):
    ...
    routes `latest`, `normal` and every room invite to `experimental`, so the PASS 84 backup
    takes that key and PASS 85 moves to `previous` for investigation. No tree is deleted by
    ...
    if rollback:
        return {
            "experimental": {
                "label": "PASS 84 · SAFE BACKUP · DEFAULT AFTER ROLLBACK",
                "description": (
                    "PASS 85 was rolled back; this is the pinned PASS 84 safe backup "
            ...
            "previous": {
                "label": "PASS 85 · ROLLED BACK",
                "description": (
                    "The PASS 85 cut, rolled back from the default. Still published here "
...
COMMIT_MESSAGE = """publish: PASS 85 - owner list of 2026-09-02, PASS 84 pinned as the single safe backup
...
carries exactly channels/pass85 (live) and channels/pass84 (safe backup); every
...
guard, in-build fallback resolves to channels/pass84.

Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>"""

ROLLBACK_COMMIT_MESSAGE = """rollback: default re-pointed to PASS 84 safe backup, PASS 85 kept for investigation

HF-400 rollback path (scripts/orchestration/publish_pass85.py --rollback). No tree
was deleted and nothing was rebuilt: channels/pass85 and channels/pass84 are both
still published; the root chooser now routes latest, normal and room invites to
channels/pass84 and offers PASS 85 as the rolled-back previous card.
...
def assert_release_identity(dist_dir):
    ...
    PASS 84 channel served an eleven-pass-old badge and release notes out of its
...
def assert_predecessors_offered(channels):
    ...
    if len(predecessors) < 1:
        sys.exit(
            ...
            "(PASS 84) beside the newest pass; publishing the newest pass beside nothing "
...
def predecessor_guard_red_test():
    ...
    try:
        assert_predecessors_offered({
            "pass85": {"pass": "PASS 85", "path": "channels/pass85"},
...
def assert_in_game_fallback_exists(worktree, post_state=None):
    """The chooser INSIDE each build must link the PASS 84 safe backup, and it must exist.

    HF-400 makes this guard stricter than its pass84 ancestor. It is no longer enough for
    the fallback to point at SOME tree on gh-pages: every tree except pass85 and pass84 is
    ...
    The fallback must resolve to channels/pass84 - the one backup the owner asked for.
...
def assert_build_is_not_stale():
    ...
    Scope relative to publish_pass84.py (pinned by src/release-topology.test.ts): the
    directory-exclusion set is the pass84 set plus `dist-pass85` and a `dist-*` prefix
    skip (a hand-copied build is never source). Nothing else is excluded - in particular
    NOT `artifacts/`: it is gitignored, but the gate's scope stays exactly the pass84
    ...
    for root, dirs, files in os.walk(SRC):
        dirs[:] = [d for d in dirs if d not in {
            "node_modules", ".git", "dist", "dist-pass83", "dist-pass84", "dist-pass85", ".gh-pages-publish",
...
def plan_retirements(gh_pages_dir):
    """HF-400: everything on gh-pages except {pass85, pass84} goes. Enumerated, not listed.

    Returns (present, to_delete). Refuses if the backup the owner asked to pin is not there
    to be pinned: retiring the rest and leaving pass85 alone would be the opposite of what
    he said.
    """
    ...
    if missing:
        sys.exit(f"REFUSING: gh-pages has no {sorted(missing)} tree to pin as the safe backup "
                 f"(present: {present}). HF-400 pins PASS 84 beside PASS 85; it cannot be "
...
def assert_post_state(gh_pages_dir):
    """After retirement, channels/ must be EXACTLY {pass85, pass84}. Read from disk."""
...
def add_to_worktree(gh_pages_dir, dry_run):
    """Copy dist-pass85 into channels/pass85 (or say that it would)."""
...
def dry_run(gh_pages_dir, plan_json, rollback=False):
    ...
    if rollback:
        ...
        # Build guards. Reported, not fatal, so a plan can be printed before dist-pass85 exists.
...
def rollback(worktree, sources):
    """Re-point the default at the PASS 84 safe backup. Deletes nothing, rebuilds nothing."""
...
def main(argv=None):
    ...
    parser.add_argument("--rollback", action="store_true",
                        help="re-point the chooser default at channels/pass84 (PASS 85 stays "
