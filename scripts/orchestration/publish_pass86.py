...
So after this script runs, gh-pages carries EXACTLY two channel trees:
    ...
    channels/pass86   the live default (this cut)
    channels/pass85   the pinned safe backup (what was live when he said it)
...
and the post-state is asserted to be exactly {pass86, pass85} before anything is committed.
...
DIST = os.path.join(SRC, "dist-pass86")
WORKTREE = os.path.join(SRC, ".gh-pages-publish")
CHANNEL = "channels/pass86"
LIVE_TREE = "pass86"
BACKUP_TREE = "pass85"
...
DESCRIPTION = (
    "Pass 86 brings the code-authored Nuke Town Rebuild preview built to the Black Ops 2 flow, Map 3 as a walkable explore showcase with eight corridors, and a first-person rig that finally sits inside the body."
)
...
# `previous` - which is exactly the alias the owner's policy needs: ?release=previous -> PASS 85.
BACKUP_COPY = {
    "label": "PASS 85 · SAFE BACKUP",
    "description": (
...
def build_channels(rollback=False):
    ...
    routes `latest`, `normal` and every room invite to `experimental`, so the PASS 85 backup
    takes that key and PASS 86 moves to `previous` for investigation. No tree is deleted by
    ...
    if rollback:
        return {
            "experimental": {
                "label": "PASS 85 · SAFE BACKUP · DEFAULT AFTER ROLLBACK",
                "description": (
                    "PASS 86 was rolled back; this is the pinned PASS 85 safe backup "
            ...
            "previous": {
                "label": "PASS 86 · ROLLED BACK",
                "description": (
                    "The PASS 86 cut, rolled back from the default. Still published here "
...
COMMIT_MESSAGE = """publish: PASS 86 - owner list of 2026-09-02, PASS 85 pinned as the single safe backup
...
carries exactly channels/pass86 (live) and channels/pass85 (safe backup); every
...
ROLLBACK_COMMIT_MESSAGE = """rollback: default re-pointed to PASS 85 safe backup, PASS 86 kept for investigation

HF-400 rollback path (scripts/orchestration/publish_pass86.py --rollback). No tree
was deleted and nothing was rebuilt: channels/pass86 and channels/pass85 are both
still published; the root chooser now routes latest, normal and room invites to
channels/pass85 and offers PASS 86 as the rolled-back previous card.
...
def assert_release_identity(dist_dir):
    ...
    PASS 85 channel served an eleven-pass-old badge and release notes out of its
...
def assert_predecessors_offered(channels):
    ...
    if len(predecessors) < 1:
        sys.exit(
            ...
            "(PASS 85) beside the newest pass; publishing the newest pass beside nothing "
...
def predecessor_guard_red_test():
    ...
    try:
        assert_predecessors_offered({
            "pass86": {"pass": "PASS 86", "path": "channels/pass86"},
...
def assert_in_game_fallback_exists(worktree, post_state=None):
    """The chooser INSIDE each build must link the PASS 85 safe backup, and it must exist.

    HF-400 makes this guard stricter than its pass85 ancestor. It is no longer enough for
    the fallback to point at SOME tree on gh-pages: every tree except pass86 and pass85 is
    ...
    The fallback must resolve to channels/pass85 - the one backup the owner asked for.
...
def assert_build_is_not_stale():
    ...
    Scope relative to publish_pass85.py (pinned by src/release-topology.test.ts): the
    directory-exclusion set is the pass85 set plus `dist-pass86` and a `dist-*` prefix
    skip (a hand-copied build is never source). Nothing else is excluded - in particular
    NOT `artifacts/`: it is gitignored, but the gate's scope stays exactly the pass85
    ...
    for root, dirs, files in os.walk(SRC):
        dirs[:] = [d for d in dirs if d not in {
            "node_modules", ".git", "dist", "dist-pass83", "dist-pass84", "dist-pass85", "dist-pass86", ".gh-pages-publish",
...
def plan_retirements(gh_pages_dir):
    """HF-400: everything on gh-pages except {pass86, pass85} goes. Enumerated, not listed.

    Returns (present, to_delete). Refuses if the backup the owner asked to pin is not there
    to be pinned: retiring the rest and leaving pass86 alone would be the opposite of what
    he said.
    """
    ...
    if missing:
        sys.exit(f"REFUSING: gh-pages has no {sorted(missing)} tree to pin as the safe backup "
                 f"(present: {present}). HF-400 pins PASS 85 beside PASS 86; it cannot be "
...
def assert_post_state(gh_pages_dir):
    """After retirement, channels/ must be EXACTLY {pass86, pass85}. Read from disk."""
...
def add_to_worktree(gh_pages_dir, dry_run):
    """Copy dist-pass86 into channels/pass86 (or say that it would)."""
...
def dry_run(gh_pages_dir, plan_json, rollback=False):
    ...
    if rollback:
        ...
        # Build guards. Reported, not fatal, so a plan can be printed before dist-pass86 exists.
...
def rollback(worktree, sources):
    """Re-point the default at the PASS 85 safe backup. Deletes nothing, rebuilds nothing."""
...
def main(argv=None):
    ...
    parser.add_argument("--rollback", action="store_true",
                        help="re-point the chooser default at channels/pass85 (PASS 86 stays "
