#!/usr/bin/env python3
"""Publish the Pass 87 candidate as its own gh-pages channel and retire every other tree.
...
Sibling of publish_pass86.py, with one policy change from the owner (HF-400).
...
So after this script runs, gh-pages carries EXACTLY two channel trees:
    ...
    channels/pass87   the live default (this cut)
    channels/pass86   the pinned safe backup (what was live when he said it)
...
and the post-state is asserted to be exactly {pass87, pass86} before anything is committed.
...
Guards kept from publish_pass86.py, unchanged in strictness: the build freshness guard
...
must resolve to the PASS 86 backup, not merely to something that exists.
...
DIST = os.path.join(SRC, "dist-pass87")
WORKTREE = os.path.join(SRC, ".gh-pages-publish")
CHANNEL = "channels/pass87"
LIVE_TREE = "pass87"
BACKUP_TREE = "pass86"
...
DESCRIPTION = (
    "Pass 87 adds the Raid Rebuild and Farcrysis preview arenas, fixes the crowded menu and the mobile pause tap, gives bots a stance, and moves the 2x core behind a real line of sight."
)
...
# `previous` - which is exactly the alias the owner's policy needs: ?release=previous -> PASS 86.
BACKUP_COPY = {
    "label": "PASS 86 · SAFE BACKUP",
    "description": (
...
def build_channels(rollback=False):
    ...
    routes `latest`, `normal` and every room invite to `experimental`, so the PASS 86 backup
    takes that key and PASS 87 moves to `previous` for investigation. No tree is deleted by
    ...
    if rollback:
        return {
            "experimental": {
                "label": "PASS 86 · SAFE BACKUP · DEFAULT AFTER ROLLBACK",
                "description": (
                    "PASS 87 was rolled back; this is the pinned PASS 86 safe backup "
            ...
            "previous": {
                "label": "PASS 87 · ROLLED BACK",
                "description": (
                    "The PASS 87 cut, rolled back from the default. Still published here "
...
COMMIT_MESSAGE = """publish: PASS 87 - owner list of 2026-09-02, PASS 86 pinned as the single safe backup
...
carries exactly channels/pass87 (live) and channels/pass86 (safe backup); every
...
ROLLBACK_COMMIT_MESSAGE = """rollback: default re-pointed to PASS 86 safe backup, PASS 87 kept for investigation

HF-400 rollback path (scripts/orchestration/publish_pass87.py --rollback). No tree
was deleted and nothing was rebuilt: channels/pass87 and channels/pass86 are both
still published; the root chooser now routes latest, normal and room invites to
channels/pass86 and offers PASS 87 as the rolled-back previous card.
...
def assert_release_identity(dist_dir):
    ...
    PASS 86 channel served an eleven-pass-old badge and release notes out of its
...
def assert_predecessors_offered(channels):
    ...
    if len(predecessors) < 1:
        sys.exit(
            ...
            "(PASS 86) beside the newest pass; publishing the newest pass beside nothing "
...
def predecessor_guard_red_test():
    ...
    try:
        assert_predecessors_offered({
            "pass87": {"pass": "PASS 87", "path": "channels/pass87"},
...
def assert_in_game_fallback_exists(worktree, post_state=None):
    """The chooser INSIDE each build must link the PASS 86 safe backup, and it must exist.

    HF-400 makes this guard stricter than its pass86 ancestor. It is no longer enough for
    the fallback to point at SOME tree on gh-pages: every tree except pass87 and pass86 is
    ...
    The fallback must resolve to channels/pass86 - the one backup the owner asked for.
...
def assert_build_is_not_stale():
    ...
    Scope relative to publish_pass86.py (pinned by src/release-topology.test.ts): the
    directory-exclusion set is the pass86 set plus `dist-pass87` and a `dist-*` prefix
    skip (a hand-copied build is never source). Nothing else is excluded - in particular
    NOT `artifacts/`: it is gitignored, but the gate's scope stays exactly the pass86
    ...
    for root, dirs, files in os.walk(SRC):
        dirs[:] = [d for d in dirs if d not in {
            "node_modules", ".git", "dist", "dist-pass83", "dist-pass84", "dist-pass85", "dist-pass86", "dist-pass87", ".gh-pages-publish",
...
def plan_retirements(gh_pages_dir):
    """HF-400: everything on gh-pages except {pass87, pass86} goes. Enumerated, not listed.

    Returns (present, to_delete). Refuses if the backup the owner asked to pin is not there
    to be pinned: retiring the rest and leaving pass87 alone would be the opposite of what
    he said.
    """
    ...
    if missing:
        sys.exit(f"REFUSING: gh-pages has no {sorted(missing)} tree to pin as the safe backup "
                 f"(present: {present}). HF-400 pins PASS 86 beside PASS 87; it cannot be "
...
def assert_post_state(gh_pages_dir):
    """After retirement, channels/ must be EXACTLY {pass87, pass86}. Read from disk."""
...
def add_to_worktree(gh_pages_dir, dry_run):
    """Copy dist-pass87 into channels/pass87 (or say that it would)."""
...
def dry_run(gh_pages_dir, plan_json, rollback=False):
    ...
    if rollback:
        ...
        # Build guards. Reported, not fatal, so a plan can be printed before dist-pass87 exists.
...
def rollback(worktree, sources):
    """Re-point the default at the PASS 86 safe backup. Deletes nothing, rebuilds nothing."""
...
def main(argv=None):
    ...
    parser.add_argument("--rollback", action="store_true",
                        help="re-point the chooser default at channels/pass86 (PASS 87 stays "
