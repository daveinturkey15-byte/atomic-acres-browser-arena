// Contract test for scripts/orchestration/publish_pass88.py --dry-run (HF-400).
//
...
// pass87, keep pass87, add pass88, offer exactly those two, and resolve the in-build
// fallback to channels/pass87 - all without writing a byte into the fake tree.
//
...
const script = join(repo, 'scripts', 'orchestration', 'publish_pass88.py');

// What origin/gh-pages carried at 718a5295 (2026-09-02, verified with git ls-tree).
const LIVE_TREES = ['pass72-retained', 'pass81', 'pass82', 'pass87', 'recent-stable', 'the-big-one'];
...
const LIVE_CONFIG = {
  ...
  pass87: { label: 'PASS 87', description: 'x', pass: 'PASS 87', path: 'channels/pass87' },
};
...
function python() {
  ...
  assert.fail('no python on PATH - the publish_pass88 contract cannot run, and an unrun contract is not a pass');
}

function fakeGhPages(trees = LIVE_TREES) {
  const dir = mkdtempSync(join(tmpdir(), 'aa-pass88-ghpages-'));
...
test('the plan deletes every tree except pass87, keeps pass87, adds pass88, and the post-state is exactly {pass87, pass88}', () => {
  ...
  try {
    ...
    assert.deepEqual(plan.treesToDelete, LIVE_TREES.filter((tree) => tree !== 'pass87'));
    assert.equal(plan.treesToDelete.length, 5);
    assert.deepEqual(plan.treesKept, ['pass87']);
    assert.equal(plan.treeAdded, 'pass88');
    assert.deepEqual(plan.postState, ['pass87', 'pass88']);
    assert.equal(plan.channel, 'channels/pass88');
    assert.equal(plan.backup, 'channels/pass87');
    ...
    assert.match(output, /would keep\s+\['channels\/pass87\/'\]/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('the chooser carries exactly two cards, keyed for the shell aliases: experimental -> PASS 88 live, previous -> PASS 87 backup', () => {
  const dir = fakeGhPages();
  try {
    ...
    assert.deepEqual(
      ...
      { pass: 'PASS 87', path: 'channels/pass87', label: 'PASS 87 · SAFE BACKUP' },
    );
    ...
    assert.deepEqual(plan.chooser.droppedLiveKeys.sort(), ['experimental', 'pass81', 'pass82', 'pass87', 'previous']);
...
test('the in-build fallback (src/bootstrap.ts via release-channels.json) resolves to channels/pass87', () => {
  // Red until src/bootstrap.ts prefers `pass87Backup`: the guard reads the real
  ...
  // offer a second card that 404s the moment the pass88 publish retires its tree.
  const dir = fakeGhPages();
  try {
    const { plan } = dryRun(dir);
    assert.equal(plan.fallback.path, 'channels/pass87',
      `in-build fallback resolves to '${plan.fallback.key}' -> ${plan.fallback.path}; HF-400 retires that tree. `
      + 'Re-pin `const stableFallback` in src/bootstrap.ts to releaseChannels.pass87Backup.');
    assert.equal(plan.fallback.pass, 'PASS 87');
...
test('refuses when the pass87 backup is not on gh-pages to be pinned, and plans no deletions', () => {
  const dir = fakeGhPages(LIVE_TREES.filter((tree) => tree !== 'pass87'));
  try {
    const { status, plan } = dryRun(dir);
    assert.equal(status, 2);
    assert.equal(plan.wouldPublish, false);
    assert.equal(plan.guards['backup-present'].ok, false);
    assert.match(plan.guards['backup-present'].detail, /no \['pass87'\] tree to pin as the safe backup/);
...
test('rollback re-points the default at PASS 87, keeps PASS 88 as the previous card, deletes nothing, needs no build', () => {
  const dir = fakeGhPages(['pass87', 'pass88']);
  try {
    const before = snapshot(dir);
    ...
    assert.equal('build-present' in plan.guards, false, 'a rollback must not depend on dist-pass88');
    assert.equal('build-freshness' in plan.guards, false, 'a rollback must not depend on dist-pass88');
    assert.deepEqual(plan.chooser.keys, ['experimental', 'previous']);
    assert.deepEqual(
      { pass: plan.chooser.channels.experimental.pass, path: plan.chooser.channels.experimental.path, deploymentState: plan.chooser.channels.experimental.deploymentState },
    ...
    assert.deepEqual(
      ...
      { pass: 'PASS 88', path: 'channels/pass88' },
    );
    assert.deepEqual(snapshot(dir), before);
  } finally {
    rmSync(dir, { recursive: true, force: true });
...
test('rollback refuses when gh-pages is not already exactly {pass87, pass88}', () => {
  const dir = fakeGhPages();
  try {
    const { status, plan } = dryRun(dir, '--rollback');
    assert.equal(status, 2);
    assert.equal(plan.guards['rollback-state'].ok, false);
    assert.match(plan.guards['rollback-state'].detail, /post-state is \[.*\], expected exactly \['pass87', 'pass88'\]/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test('the outside-ownership patch is tracked and either applies cleanly or is already applied', () => {
  // Case 3 above is red until src/bootstrap.ts prefers `pass87Backup`. That change is
  ...
  const patch = join(repo, 'docs', 'pass88-outside-ownership.patch');
  assert.ok(statSync(patch).isFile(), `${patch} is missing`);
  const text = readFileSync(patch, 'utf8');
  assert.match(text, /^\+const stableFallback = releaseChannels\.pass87Backup \?\? releaseChannels\.stable;$/m);
  assert.match(text, /^\+  pass87Backup\?: Readonly</m);
  const tracked = spawnSync('git', ['ls-files', '--error-unmatch', 'docs/pass88-outside-ownership.patch'], { cwd: repo, encoding: 'utf8' });
  assert.equal(tracked.status, 0, 'docs/pass88-outside-ownership.patch must be tracked so it survives the merge');
