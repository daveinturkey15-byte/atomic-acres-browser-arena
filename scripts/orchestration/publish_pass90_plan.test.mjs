// Contract test for scripts/orchestration/publish_pass90.py --dry-run (HF-400).
//
...
// pass89, keep pass89, add pass90, offer exactly those two, and resolve the in-build
// fallback to channels/pass89 - all without writing a byte into the fake tree.
//
...
const script = join(repo, 'scripts', 'orchestration', 'publish_pass90.py');

// What origin/gh-pages carried at 718a5295 (2026-09-02, verified with git ls-tree).
const LIVE_TREES = ['pass72-retained', 'pass81', 'pass82', 'pass89', 'recent-stable', 'the-big-one'];
...
const LIVE_CONFIG = {
  ...
  pass89: { label: 'PASS 89', description: 'x', pass: 'PASS 89', path: 'channels/pass89' },
};
...
function python() {
  ...
  assert.fail('no python on PATH - the publish_pass90 contract cannot run, and an unrun contract is not a pass');
}

function fakeGhPages(trees = LIVE_TREES) {
  const dir = mkdtempSync(join(tmpdir(), 'aa-pass90-ghpages-'));
...
test('the plan deletes every tree except pass89, keeps pass89, adds pass90, and the post-state is exactly {pass89, pass90}', () => {
  ...
  try {
    ...
    assert.deepEqual(plan.treesToDelete, LIVE_TREES.filter((tree) => tree !== 'pass89'));
    assert.equal(plan.treesToDelete.length, 5);
    assert.deepEqual(plan.treesKept, ['pass89']);
    assert.equal(plan.treeAdded, 'pass90');
    assert.deepEqual(plan.postState, ['pass89', 'pass90']);
    assert.equal(plan.channel, 'channels/pass90');
    assert.equal(plan.backup, 'channels/pass89');
    ...
    assert.match(output, /would keep\s+\['channels\/pass89\/'\]/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('the chooser carries exactly two cards, keyed for the shell aliases: experimental -> PASS 90 live, previous -> PASS 89 backup', () => {
  const dir = fakeGhPages();
  try {
    ...
    assert.deepEqual(
      ...
      { pass: 'PASS 89', path: 'channels/pass89', label: 'PASS 89 · SAFE BACKUP' },
    );
    ...
    assert.deepEqual(plan.chooser.droppedLiveKeys.sort(), ['experimental', 'pass81', 'pass82', 'pass89', 'previous']);
...
test('the in-build fallback (src/bootstrap.ts via release-channels.json) resolves to channels/pass89', () => {
  // Red until src/bootstrap.ts prefers `pass89Backup`: the guard reads the real
  ...
  // offer a second card that 404s the moment the pass90 publish retires its tree.
  const dir = fakeGhPages();
  try {
    const { plan } = dryRun(dir);
    assert.equal(plan.fallback.path, 'channels/pass89',
      `in-build fallback resolves to '${plan.fallback.key}' -> ${plan.fallback.path}; HF-400 retires that tree. `
      + 'Re-pin `const stableFallback` in src/bootstrap.ts to releaseChannels.pass89Backup.');
    assert.equal(plan.fallback.pass, 'PASS 89');
...
test('refuses when the pass89 backup is not on gh-pages to be pinned, and plans no deletions', () => {
  const dir = fakeGhPages(LIVE_TREES.filter((tree) => tree !== 'pass89'));
  try {
    const { status, plan } = dryRun(dir);
    assert.equal(status, 2);
    assert.equal(plan.wouldPublish, false);
    assert.equal(plan.guards['backup-present'].ok, false);
    assert.match(plan.guards['backup-present'].detail, /no \['pass89'\] tree to pin as the safe backup/);
...
test('rollback re-points the default at PASS 89, keeps PASS 90 as the previous card, deletes nothing, needs no build', () => {
  const dir = fakeGhPages(['pass89', 'pass90']);
  try {
    const before = snapshot(dir);
    ...
    assert.equal('build-present' in plan.guards, false, 'a rollback must not depend on dist-pass90');
    assert.equal('build-freshness' in plan.guards, false, 'a rollback must not depend on dist-pass90');
    assert.deepEqual(plan.chooser.keys, ['experimental', 'previous']);
    assert.deepEqual(
      { pass: plan.chooser.channels.experimental.pass, path: plan.chooser.channels.experimental.path, deploymentState: plan.chooser.channels.experimental.deploymentState },
    ...
    assert.deepEqual(
      ...
      { pass: 'PASS 90', path: 'channels/pass90' },
    );
    assert.deepEqual(snapshot(dir), before);
  } finally {
    rmSync(dir, { recursive: true, force: true });
...
test('rollback refuses when gh-pages is not already exactly {pass89, pass90}', () => {
  const dir = fakeGhPages();
  try {
    const { status, plan } = dryRun(dir, '--rollback');
    assert.equal(status, 2);
    assert.equal(plan.guards['rollback-state'].ok, false);
    assert.match(plan.guards['rollback-state'].detail, /post-state is \[.*\], expected exactly \['pass89', 'pass90'\]/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test('the outside-ownership patch is tracked and either applies cleanly or is already applied', () => {
  // Case 3 above is red until src/bootstrap.ts prefers `pass89Backup`. That change is
  ...
  const patch = join(repo, 'docs', 'pass90-outside-ownership.patch');
  assert.ok(statSync(patch).isFile(), `${patch} is missing`);
  const text = readFileSync(patch, 'utf8');
  assert.match(text, /^\+const stableFallback = releaseChannels\.pass89Backup \?\? releaseChannels\.stable;$/m);
  assert.match(text, /^\+  pass89Backup\?: Readonly</m);
  const tracked = spawnSync('git', ['ls-files', '--error-unmatch', 'docs/pass90-outside-ownership.patch'], { cwd: repo, encoding: 'utf8' });
  assert.equal(tracked.status, 0, 'docs/pass90-outside-ownership.patch must be tracked so it survives the merge');
