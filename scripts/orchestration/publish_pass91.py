#!/usr/bin/env python3
"""Publish the Pass 91 candidate as its own gh-pages channel and retire every other tree.
...
Sibling of publish_pass90.py, with one policy change from the owner (HF-400).
...
So after this script runs, gh-pages carries EXACTLY two channel trees:
    ...
    channels/pass91   the live default (this cut)
    channels/pass90   the pinned safe backup (what was live when he said it)
...
and the post-state is asserted to be exactly {pass91, pass90} before anything is committed.
...
Guards kept from publish_pass90.py, unchanged in strictness: the build freshness guard
...
must resolve to the PASS 90 backup, not merely to something that exists.
...
DIST = os.path.join(SRC, "dist-pass91")
WORKTREE = os.path.join(SRC, ".gh-pages-publish")
CHANNEL = "channels/pass91"
LIVE_TREE = "pass91"
BACKUP_TREE = "pass90"
...
DESCRIPTION = (
    "Pass 91 refines the Nuke Town Rebuild from the owner's play - stairs, side areas, spawns, walk-through doors and the street vehicles where the reference has them - and gives crouching its own slower speed with no sprint."
)
...
# `previous` - which is exactly the alias the owner's policy needs: ?release=previous -> PASS 90.
BACKUP_COPY = {
    "label": "PASS 90 · SAFE BACKUP",
    "description": (
...
def build_channels(rollback=False):
    ...
    routes `latest`, `normal` and every room invite to `experimental`, so the PASS 90 backup
    takes that key and PASS 91 moves to `previous` for investigation. No tree is deleted by
    ...
    if rollback:
        return {
            "experimental": {
                "label": "PASS 90 · SAFE BACKUP · DEFAULT AFTER ROLLBACK",
                "description": (
                    "PASS 91 was rolled back; this is the pinned PASS 90 safe backup "
            ...
            "previous": {
                "label": "PASS 91 · ROLLED BACK",
                "description": (
                    "The PASS 91 cut, rolled back from the default. Still published here "
...
COMMIT_MESSAGE = """publish: PASS 91 - owner list of 2026-09-02, PASS 90 pinned as the single safe backup
...
carries exactly channels/pass91 (live) and channels/pass90 (safe backup); every
...
ROLLBACK_COMMIT_MESSAGE = """rollback: default re-pointed to PASS 90 safe backup, PASS 91 kept for investigation

HF-400 rollback path (scripts/orchestration/publish_pass91.py --rollback). No tree
was deleted and nothing was rebuilt: channels/pass91 and channels/pass90 are both
still published; the root chooser now routes latest, normal and room invites to
channels/pass90 and offers PASS 91 as the rolled-back previous card.
...
def assert_release_identity(dist_dir):
    ...
    PASS 90 channel served an eleven-pass-old badge and release notes out of its
...
def assert_predecessors_offered(channels):
    ...
    if len(predecessors) < 1:
        sys.exit(
            ...
            "(PASS 90) beside the newest pass; publishing the newest pass beside nothing "
...
def predecessor_guard_red_test():
    ...
    try:
        assert_predecessors_offered({
            "pass91": {"pass": "PASS 91", "path": "channels/pass91"},
...
def assert_in_game_fallback_exists(worktree, post_state=None):
    """The chooser INSIDE each build must link the PASS 90 safe backup, and it must exist.

    HF-400 makes this guard stricter than its pass90 ancestor. It is no longer enough for
    the fallback to point at SOME tree on gh-pages: every tree except pass91 and pass90 is
    ...
    The fallback must resolve to channels/pass90 - the one backup the owner asked for.
...
def assert_build_is_not_stale():
    ...
    Scope relative to publish_pass90.py (pinned by src/release-topology.test.ts): the
    directory-exclusion set is the pass90 set plus `dist-pass91` and a `dist-*` prefix
    skip (a hand-copied build is never source). Nothing else is excluded - in particular
    NOT `artifacts/`: it is gitignored, but the gate's scope stays exactly the pass90
    ...
    for root, dirs, files in os.walk(SRC):
        dirs[:] = [d for d in dirs if d not in {
            "node_modules", ".git", "dist", "dist-pass83", "dist-pass84", "dist-pass85", "dist-pass86", "dist-pass87", "dist-pass88", "dist-pass89", "dist-pass90", "dist-pass91", ".gh-pages-publish",
...
def plan_retirements(gh_pages_dir):
    """HF-400: everything on gh-pages except {pass91, pass90} goes. Enumerated, not listed.

    Returns (present, to_delete). Refuses if the backup the owner asked to pin is not there
    to be pinned: retiring the rest and leaving pass91 alone would be the opposite of what
    he said.
    """
    ...
    if missing:
        sys.exit(f"REFUSING: gh-pages has no {sorted(missing)} tree to pin as the safe backup "
                 f"(present: {present}). HF-400 pins PASS 90 beside PASS 91; it cannot be "
...
def assert_post_state(gh_pages_dir):
    """After retirement, channels/ must be EXACTLY {pass91, pass90}. Read from disk."""
...
def add_to_worktree(gh_pages_dir, dry_run):
    """Copy dist-pass91 into channels/pass91 (or say that it would)."""
...
def dry_run(gh_pages_dir, plan_json, rollback=False):
    ...
    if rollback:
        ...
        # Build guards. Reported, not fatal, so a plan can be printed before dist-pass91 exists.
...
def rollback(worktree, sources):
    """Re-point the default at the PASS 90 safe backup. Deletes nothing, rebuilds nothing."""
...
def main(argv=None):
    ...
    parser.add_argument("--rollback", action="store_true",
                        help="re-point the chooser default at channels/pass90 (PASS 91 stays "
