// Contract test for scripts/orchestration/publish_pass91.py --dry-run (HF-400).
//
...
// pass90, keep pass90, add pass91, offer exactly those two, and resolve the in-build
// fallback to channels/pass90 - all without writing a byte into the fake tree.
//
...
const script = join(repo, 'scripts', 'orchestration', 'publish_pass91.py');

// What origin/gh-pages carried at 718a5295 (2026-09-02, verified with git ls-tree).
const LIVE_TREES = ['pass72-retained', 'pass81', 'pass82', 'pass90', 'recent-stable', 'the-big-one'];
...
const LIVE_CONFIG = {
  ...
  pass90: { label: 'PASS 90', description: 'x', pass: 'PASS 90', path: 'channels/pass90' },
};
...
function python() {
  ...
  assert.fail('no python on PATH - the publish_pass91 contract cannot run, and an unrun contract is not a pass');
}

function fakeGhPages(trees = LIVE_TREES) {
  const dir = mkdtempSync(join(tmpdir(), 'aa-pass91-ghpages-'));
...
test('the plan deletes every tree except pass90, keeps pass90, adds pass91, and the post-state is exactly {pass90, pass91}', () => {
  ...
  try {
    ...
    assert.deepEqual(plan.treesToDelete, LIVE_TREES.filter((tree) => tree !== 'pass90'));
    assert.equal(plan.treesToDelete.length, 5);
    assert.deepEqual(plan.treesKept, ['pass90']);
    assert.equal(plan.treeAdded, 'pass91');
    assert.deepEqual(plan.postState, ['pass90', 'pass91']);
    assert.equal(plan.channel, 'channels/pass91');
    assert.equal(plan.backup, 'channels/pass90');
    ...
    assert.match(output, /would keep\s+\['channels\/pass90\/'\]/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('the chooser carries exactly two cards, keyed for the shell aliases: experimental -> PASS 91 live, previous -> PASS 90 backup', () => {
  const dir = fakeGhPages();
  try {
    ...
    assert.deepEqual(
      ...
      { pass: 'PASS 91', path: 'channels/pass91', deploymentState: 'live' },
    );
    assert.deepEqual(
      ...
      { pass: 'PASS 90', path: 'channels/pass90', label: 'PASS 90 · SAFE BACKUP' },
    );
    ...
    assert.deepEqual(plan.chooser.droppedLiveKeys.sort(), ['experimental', 'pass81', 'pass82', 'pass90', 'previous']);
...
test('the in-build fallback (src/bootstrap.ts via release-channels.json) resolves to channels/pass90', () => {
  // Red until src/bootstrap.ts prefers `pass90Backup`: the guard reads the real
  ...
  // offer a second card that 404s the moment the pass91 publish retires its tree.
  const dir = fakeGhPages();
  try {
    const { plan } = dryRun(dir);
    assert.equal(plan.fallback.path, 'channels/pass90',
      `in-build fallback resolves to '${plan.fallback.key}' -> ${plan.fallback.path}; HF-400 retires that tree. `
      + 'Re-pin `const stableFallback` in src/bootstrap.ts to releaseChannels.pass90Backup.');
    assert.equal(plan.fallback.pass, 'PASS 90');
...
test('refuses when the pass90 backup is not on gh-pages to be pinned, and plans no deletions', () => {
  const dir = fakeGhPages(LIVE_TREES.filter((tree) => tree !== 'pass90'));
  try {
    const { status, plan } = dryRun(dir);
    assert.equal(status, 2);
    assert.equal(plan.wouldPublish, false);
    assert.equal(plan.guards['backup-present'].ok, false);
    assert.match(plan.guards['backup-present'].detail, /no \['pass90'\] tree to pin as the safe backup/);
...
test('rollback re-points the default at PASS 90, keeps PASS 91 as the previous card, deletes nothing, needs no build', () => {
  const dir = fakeGhPages(['pass90', 'pass91']);
  try {
    const before = snapshot(dir);
    ...
    assert.equal('build-present' in plan.guards, false, 'a rollback must not depend on dist-pass91');
    assert.equal('build-freshness' in plan.guards, false, 'a rollback must not depend on dist-pass91');
    assert.deepEqual(plan.chooser.keys, ['experimental', 'previous']);
    assert.deepEqual(
      { pass: plan.chooser.channels.experimental.pass, path: plan.chooser.channels.experimental.path, deploymentState: plan.chooser.channels.experimental.deploymentState },
      { pass: 'PASS 90', path: 'channels/pass90', deploymentState: 'live' },
    );
    assert.deepEqual(
      { pass: plan.chooser.channels.previous.pass, path: plan.chooser.channels.previous.path },
      { pass: 'PASS 91', path: 'channels/pass91' },
    );
    assert.deepEqual(snapshot(dir), before);
  } finally {
    rmSync(dir, { recursive: true, force: true });
...
test('rollback refuses when gh-pages is not already exactly {pass90, pass91}', () => {
  const dir = fakeGhPages();
  try {
    const { status, plan } = dryRun(dir, '--rollback');
    assert.equal(status, 2);
    assert.equal(plan.guards['rollback-state'].ok, false);
    assert.match(plan.guards['rollback-state'].detail, /post-state is \[.*\], expected exactly \['pass90', 'pass91'\]/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test('the outside-ownership patch is tracked and either applies cleanly or is already applied', () => {
  // Case 3 above is red until src/bootstrap.ts prefers `pass90Backup`. That change is
  ...
  const patch = join(repo, 'docs', 'pass91-outside-ownership.patch');
  assert.ok(statSync(patch).isFile(), `${patch} is missing`);
  const text = readFileSync(patch, 'utf8');
  assert.match(text, /^\+const stableFallback = releaseChannels\.pass90Backup \?\? releaseChannels\.stable;$/m);
  assert.match(text, /^\+  pass90Backup\?: Readonly</m);
  const tracked = spawnSync('git', ['ls-files', '--error-unmatch', 'docs/pass91-outside-ownership.patch'], { cwd: repo, encoding: 'utf8' });
  assert.equal(tracked.status, 0, 'docs/pass91-outside-ownership.patch must be tracked so it survives the merge');
