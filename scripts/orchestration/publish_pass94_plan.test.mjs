// Contract test for scripts/orchestration/publish_pass94.py --dry-run (HF-400).
//
...
// pass93, keep pass93, add pass94, offer exactly those two, and resolve the in-build
// fallback to channels/pass93 - all without writing a byte into the fake tree.
//
...
const script = join(repo, 'scripts', 'orchestration', 'publish_pass94.py');

// What origin/gh-pages carried at 718a5295 (2026-09-02, verified with git ls-tree).
const LIVE_TREES = ['pass72-retained', 'pass81', 'pass82', 'pass93', 'recent-stable', 'the-big-one'];
...
const LIVE_CONFIG = {
  ...
  pass93: { label: 'PASS 93', description: 'x', pass: 'PASS 93', path: 'channels/pass93' },
};
...
function python() {
  ...
  assert.fail('no python on PATH - the publish_pass94 contract cannot run, and an unrun contract is not a pass');
}

function fakeGhPages(trees = LIVE_TREES) {
  const dir = mkdtempSync(join(tmpdir(), 'aa-pass94-ghpages-'));
...
test('the plan deletes every tree except pass93, keeps pass93, adds pass94, and the post-state is exactly {pass93, pass94}', () => {
  ...
  try {
    ...
    assert.deepEqual(plan.treesToDelete, LIVE_TREES.filter((tree) => tree !== 'pass93'));
    assert.equal(plan.treesToDelete.length, 5);
    assert.deepEqual(plan.treesKept, ['pass93']);
    assert.equal(plan.treeAdded, 'pass94');
    assert.deepEqual(plan.postState, ['pass93', 'pass94']);
    assert.equal(plan.channel, 'channels/pass94');
    assert.equal(plan.backup, 'channels/pass93');
    ...
    assert.match(output, /would keep\s+\['channels\/pass93\/'\]/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('the chooser carries exactly two cards, keyed for the shell aliases: experimental -> PASS 94 live, previous -> PASS 93 backup', () => {
  const dir = fakeGhPages();
  try {
    ...
    assert.deepEqual(
      ...
      { pass: 'PASS 94', path: 'channels/pass94', deploymentState: 'live' },
    );
    assert.deepEqual(
      ...
      { pass: 'PASS 93', path: 'channels/pass93', label: 'PASS 93 · SAFE BACKUP' },
    );
    ...
    assert.deepEqual(plan.chooser.droppedLiveKeys.sort(), ['experimental', 'pass81', 'pass82', 'pass93', 'previous']);
...
test('the in-build fallback (src/bootstrap.ts via release-channels.json) resolves to channels/pass93', () => {
  // Red until src/bootstrap.ts prefers `pass93Backup`: the guard reads the real
  ...
  // offer a second card that 404s the moment the pass94 publish retires its tree.
  const dir = fakeGhPages();
  try {
    const { plan } = dryRun(dir);
    assert.equal(plan.fallback.path, 'channels/pass93',
      `in-build fallback resolves to '${plan.fallback.key}' -> ${plan.fallback.path}; HF-400 retires that tree. `
      + 'Re-pin `const stableFallback` in src/bootstrap.ts to releaseChannels.pass93Backup.');
    assert.equal(plan.fallback.pass, 'PASS 93');
...
test('refuses when the pass93 backup is not on gh-pages to be pinned, and plans no deletions', () => {
  const dir = fakeGhPages(LIVE_TREES.filter((tree) => tree !== 'pass93'));
  try {
    const { status, plan } = dryRun(dir);
    assert.equal(status, 2);
    assert.equal(plan.wouldPublish, false);
    assert.equal(plan.guards['backup-present'].ok, false);
    assert.match(plan.guards['backup-present'].detail, /no \['pass93'\] tree to pin as the safe backup/);
...
test('rollback re-points the default at PASS 93, keeps PASS 94 as the previous card, deletes nothing, needs no build', () => {
  const dir = fakeGhPages(['pass93', 'pass94']);
  try {
    const before = snapshot(dir);
    ...
    assert.equal('build-present' in plan.guards, false, 'a rollback must not depend on dist-pass94');
    assert.equal('build-freshness' in plan.guards, false, 'a rollback must not depend on dist-pass94');
    assert.deepEqual(plan.chooser.keys, ['experimental', 'previous']);
    assert.deepEqual(
      { pass: plan.chooser.channels.experimental.pass, path: plan.chooser.channels.experimental.path, deploymentState: plan.chooser.channels.experimental.deploymentState },
      { pass: 'PASS 93', path: 'channels/pass93', deploymentState: 'live' },
    );
    assert.deepEqual(
      { pass: plan.chooser.channels.previous.pass, path: plan.chooser.channels.previous.path },
      { pass: 'PASS 94', path: 'channels/pass94' },
    );
    assert.deepEqual(snapshot(dir), before);
  } finally {
    rmSync(dir, { recursive: true, force: true });
...
test('rollback refuses when gh-pages is not already exactly {pass93, pass94}', () => {
  const dir = fakeGhPages();
  try {
    const { status, plan } = dryRun(dir, '--rollback');
    assert.equal(status, 2);
    assert.equal(plan.guards['rollback-state'].ok, false);
    assert.match(plan.guards['rollback-state'].detail, /post-state is \[.*\], expected exactly \['pass93', 'pass94'\]/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test('the outside-ownership patch is tracked and either applies cleanly or is already applied', () => {
  // Case 3 above is red until src/bootstrap.ts prefers `pass93Backup`. That change is
  ...
  const patch = join(repo, 'docs', 'pass94-outside-ownership.patch');
  assert.ok(statSync(patch).isFile(), `${patch} is missing`);
  const text = readFileSync(patch, 'utf8');
  assert.match(text, /^\+const stableFallback = releaseChannels\.pass93Backup \?\? releaseChannels\.stable;$/m);
  assert.match(text, /^\+  pass93Backup\?: Readonly</m);
  const tracked = spawnSync('git', ['ls-files', '--error-unmatch', 'docs/pass94-outside-ownership.patch'], { cwd: repo, encoding: 'utf8' });
  assert.equal(tracked.status, 0, 'docs/pass94-outside-ownership.patch must be tracked so it survives the merge');
