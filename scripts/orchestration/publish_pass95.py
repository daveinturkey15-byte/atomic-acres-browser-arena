#!/usr/bin/env python3
"""Publish the Pass 95 candidate as its own gh-pages channel and retire every other tree.
...
Sibling of publish_pass94.py, with one policy change from the owner (HF-400).
...
So after this script runs, gh-pages carries EXACTLY two channel trees:
    ...
    channels/pass95   the live default (this cut)
    channels/pass94   the pinned safe backup (what was live when he said it)
...
and the post-state is asserted to be exactly {pass95, pass94} before anything is committed.
...
Guards kept from publish_pass94.py, unchanged in strictness: the build freshness guard
...
must resolve to the PASS 94 backup, not merely to something that exists.
...
DIST = os.path.join(SRC, "dist-pass95")
WORKTREE = os.path.join(SRC, ".gh-pages-publish")
CHANNEL = "channels/pass95"
LIVE_TREE = "pass95"
BACKUP_TREE = "pass94"
...
DESCRIPTION = (
    "Nuke Town gains image-generated PBR relief on house and vehicle surfaces, a taller textured massif with a LOD conifer belt, killstreak audio that no longer drops cues, a duplicate activation-frame walk removed, and a codebase refactor that deletes 435 lines of re-export barrels."
)
...
# `previous` - which is exactly the alias the owner's policy needs: ?release=previous -> PASS 94.
BACKUP_COPY = {
    "label": "PASS 94 · SAFE BACKUP",
    "description": (
...
def build_channels(rollback=False):
    ...
    routes `latest`, `normal` and every room invite to `experimental`, so the PASS 94 backup
    takes that key and PASS 95 moves to `previous` for investigation. No tree is deleted by
    ...
    if rollback:
        return {
            "experimental": {
                "label": "PASS 94 · SAFE BACKUP · DEFAULT AFTER ROLLBACK",
                "description": (
                    "PASS 95 was rolled back; this is the pinned PASS 94 safe backup "
            ...
            "previous": {
                "label": "PASS 95 · ROLLED BACK",
                "description": (
                    "The PASS 95 cut, rolled back from the default. Still published here "
...
COMMIT_MESSAGE = """publish: PASS 95 - owner list of 2026-09-02, PASS 94 pinned as the single safe backup
...
carries exactly channels/pass95 (live) and channels/pass94 (safe backup); every
...
ROLLBACK_COMMIT_MESSAGE = """rollback: default re-pointed to PASS 94 safe backup, PASS 95 kept for investigation

HF-400 rollback path (scripts/orchestration/publish_pass95.py --rollback). No tree
was deleted and nothing was rebuilt: channels/pass95 and channels/pass94 are both
still published; the root chooser now routes latest, normal and room invites to
channels/pass94 and offers PASS 95 as the rolled-back previous card.
...
def assert_release_identity(dist_dir):
    ...
    PASS 94 channel served an eleven-pass-old badge and release notes out of its
...
def assert_predecessors_offered(channels):
    ...
    if len(predecessors) < 1:
        sys.exit(
            ...
            "(PASS 94) beside the newest pass; publishing the newest pass beside nothing "
...
def predecessor_guard_red_test():
    ...
    try:
        assert_predecessors_offered({
            "pass95": {"pass": "PASS 95", "path": "channels/pass95"},
...
def assert_in_game_fallback_exists(worktree, post_state=None):
    """The chooser INSIDE each build must link the PASS 94 safe backup, and it must exist.

    HF-400 makes this guard stricter than its pass94 ancestor. It is no longer enough for
    the fallback to point at SOME tree on gh-pages: every tree except pass95 and pass94 is
    ...
    The fallback must resolve to channels/pass94 - the one backup the owner asked for.
...
def assert_build_is_not_stale():
    ...
    Scope relative to publish_pass94.py (pinned by src/release-topology.test.ts): the
    directory-exclusion set is the pass94 set plus `dist-pass95` and a `dist-*` prefix
    skip (a hand-copied build is never source). Nothing else is excluded - in particular
    NOT `artifacts/`: it is gitignored, but the gate's scope stays exactly the pass94
    ...
    for root, dirs, files in os.walk(SRC):
        dirs[:] = [d for d in dirs if d not in {
            "node_modules", ".git", "dist", "dist-pass83", "dist-pass84", "dist-pass85", "dist-pass86", "dist-pass87", "dist-pass88", "dist-pass89", "dist-pass90", "dist-pass91", "dist-pass92", "dist-pass93", "dist-pass94", "dist-pass95", ".gh-pages-publish",
...
def plan_retirements(gh_pages_dir):
    """HF-400: everything on gh-pages except {pass95, pass94} goes. Enumerated, not listed.

    Returns (present, to_delete). Refuses if the backup the owner asked to pin is not there
    to be pinned: retiring the rest and leaving pass95 alone would be the opposite of what
    he said.
    """
    ...
    if missing:
        sys.exit(f"REFUSING: gh-pages has no {sorted(missing)} tree to pin as the safe backup "
                 f"(present: {present}). HF-400 pins PASS 94 beside PASS 95; it cannot be "
...
def assert_post_state(gh_pages_dir):
    """After retirement, channels/ must be EXACTLY {pass95, pass94}. Read from disk."""
...
def add_to_worktree(gh_pages_dir, dry_run):
    """Copy dist-pass95 into channels/pass95 (or say that it would)."""
...
def dry_run(gh_pages_dir, plan_json, rollback=False):
    ...
    if rollback:
        ...
        # Build guards. Reported, not fatal, so a plan can be printed before dist-pass95 exists.
...
def rollback(worktree, sources):
    """Re-point the default at the PASS 94 safe backup. Deletes nothing, rebuilds nothing."""
...
def main(argv=None):
    ...
    parser.add_argument("--rollback", action="store_true",
                        help="re-point the chooser default at channels/pass94 (PASS 95 stays "
