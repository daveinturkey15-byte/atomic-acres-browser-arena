...
So after this script runs, gh-pages carries EXACTLY two channel trees:
    ...
    channels/pass96   the live default (this cut)
    channels/pass95   the pinned safe backup (what was live when he said it)
...
and the post-state is asserted to be exactly {pass96, pass95} before anything is committed.
...
Guards kept from publish_pass95.py, unchanged in strictness: the build freshness guard
...
must resolve to the PASS 95 backup, not merely to something that exists.
...
DIST = os.path.join(SRC, "dist-pass96")
WORKTREE = os.path.join(SRC, ".gh-pages-publish")
CHANNEL = "channels/pass96"
LIVE_TREE = "pass96"
BACKUP_TREE = "pass95"
...
# `previous` - which is exactly the alias the owner's policy needs: ?release=previous -> PASS 95.
BACKUP_COPY = {
    "label": "PASS 95 · SAFE BACKUP",
    "description": (
...
def build_channels(rollback=False):
    ...
    routes `latest`, `normal` and every room invite to `experimental`, so the PASS 95 backup
    takes that key and PASS 96 moves to `previous` for investigation. No tree is deleted by
    ...
    if rollback:
        return {
            "experimental": {
                "label": "PASS 95 · SAFE BACKUP · DEFAULT AFTER ROLLBACK",
                "description": (
                    "PASS 96 was rolled back; this is the pinned PASS 95 safe backup "
            ...
            "previous": {
                "label": "PASS 96 · ROLLED BACK",
                "description": (
                    "The PASS 96 cut, rolled back from the default. Still published here "
...
COMMIT_MESSAGE = """publish: PASS 96 - owner list of 2026-09-02, PASS 95 pinned as the single safe backup
...
carries exactly channels/pass96 (live) and channels/pass95 (safe backup); every
...
ROLLBACK_COMMIT_MESSAGE = """rollback: default re-pointed to PASS 95 safe backup, PASS 96 kept for investigation

HF-400 rollback path (scripts/orchestration/publish_pass96.py --rollback). No tree
was deleted and nothing was rebuilt: channels/pass96 and channels/pass95 are both
still published; the root chooser now routes latest, normal and room invites to
channels/pass95 and offers PASS 96 as the rolled-back previous card.
...
def assert_release_identity(dist_dir):
    ...
    PASS 95 channel served an eleven-pass-old badge and release notes out of its
...
def assert_predecessors_offered(channels):
    ...
    if len(predecessors) < 1:
        sys.exit(
            ...
            "(PASS 95) beside the newest pass; publishing the newest pass beside nothing "
...
def predecessor_guard_red_test():
    ...
    try:
        assert_predecessors_offered({
            "pass96": {"pass": "PASS 96", "path": "channels/pass96"},
...
def assert_in_game_fallback_exists(worktree, post_state=None):
    """The chooser INSIDE each build must link the PASS 95 safe backup, and it must exist.

    HF-400 makes this guard stricter than its pass95 ancestor. It is no longer enough for
    the fallback to point at SOME tree on gh-pages: every tree except pass96 and pass95 is
    ...
    The fallback must resolve to channels/pass95 - the one backup the owner asked for.
...
def assert_build_is_not_stale():
    ...
    Scope relative to publish_pass95.py (pinned by src/release-topology.test.ts): the
    directory-exclusion set is the pass95 set plus `dist-pass96` and a `dist-*` prefix
    skip (a hand-copied build is never source). Nothing else is excluded - in particular
    NOT `artifacts/`: it is gitignored, but the gate's scope stays exactly the pass95
    ...
    for root, dirs, files in os.walk(SRC):
        dirs[:] = [d for d in dirs if d not in {
            "node_modules", ".git", "dist", "dist-pass83", "dist-pass84", "dist-pass85", "dist-pass86", "dist-pass87", "dist-pass88", "dist-pass89", "dist-pass90", "dist-pass91", "dist-pass92", "dist-pass93", "dist-pass94", "dist-pass95", "dist-pass96", ".gh-pages-publish",
...
def plan_retirements(gh_pages_dir):
    """HF-400: everything on gh-pages except {pass96, pass95} goes. Enumerated, not listed.

    Returns (present, to_delete). Refuses if the backup the owner asked to pin is not there
    to be pinned: retiring the rest and leaving pass96 alone would be the opposite of what
    he said.
    """
    ...
    if missing:
        sys.exit(f"REFUSING: gh-pages has no {sorted(missing)} tree to pin as the safe backup "
                 f"(present: {present}). HF-400 pins PASS 95 beside PASS 96; it cannot be "
...
def assert_post_state(gh_pages_dir):
    """After retirement, channels/ must be EXACTLY {pass96, pass95}. Read from disk."""
...
def add_to_worktree(gh_pages_dir, dry_run):
    """Copy dist-pass96 into channels/pass96 (or say that it would)."""
...
def dry_run(gh_pages_dir, plan_json, rollback=False):
    ...
    if rollback:
        ...
        # Build guards. Reported, not fatal, so a plan can be printed before dist-pass96 exists.
...
def rollback(worktree, sources):
    """Re-point the default at the PASS 95 safe backup. Deletes nothing, rebuilds nothing."""
...
def main(argv=None):
    ...
    parser.add_argument("--rollback", action="store_true",
                        help="re-point the chooser default at channels/pass95 (PASS 96 stays "
