...
export async function loadArenaFactories(): Promise<ArenaFactories> {
  if (!factoriesPromise) {
    factoriesPromise = (async () => {
      const [{ buildArena }, { buildGunRange, buildRustworks1v1, buildSkylineTerminal }, { buildFarcrysis }, { buildHighSeas }, { addNeighbourhoodLife, loadArenaArt }, { buildTest1, buildTest2 }, { buildMap3 }, { buildNuketown2 }, { buildRaid2 }, { buildNewworldPrime }] = await Promise.all([
        import('../../src/map'),
        ...
        // PASS 97 (2026-09-14): New World Prime Day-1 standby blockout joins
        // the audit from its first commit, so it can never ship un-audited.
        import('../../src/newworld-prime-arena'),
      ]);
      ...
      return {
        ...
        // PASS 97 (2026-09-14): New World Prime Day-1 standby blockout joins
        // the audit from its first commit, so it can never ship un-audited.
        'newworld-prime': { build: buildNewworldPrime as ArenaBuild },
