...
async function main(): Promise<void> {
  ...
  const receipt = {
    ...
    loops: layout.FARCRYSIS_LOOPS.map((l) => ({ id: l.id, chebyshevM: l.chebyshevM, widthM: l.widthM, waypoints: l.waypoints, sprintLapS: Number(l.sprintLapS.toFixed(1)), register: l.register })),
    crossLanes: layout.FARCRYSIS_CROSS_LANES,
    routeSegments: layout.FARCRYSIS_ROUTE_SEGMENTS.map((edge) => ({
      ...
      sprintSeconds: Number(edge.sprintSeconds.toFixed(2)),
    })),
    spawnZones: layout.FARCRYSIS_SPAWN_ZONES,
