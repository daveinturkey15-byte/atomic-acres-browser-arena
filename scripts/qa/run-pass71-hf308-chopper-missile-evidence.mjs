...
import {
  ...
  PASS71_HF308_MECHANICAL_TEST_FILES,
  PASS71_HF308_POLICY,
  PASS71_HF308_REQUIRED_ASSERTIONS,
  PASS71_HF308_SCOPES,
  pass71Hf308EvidenceFailures,
  pass71Hf308RecordSha256,
  pass71Hf308ToolingHashesAtSource,
...
const firstPreviewPort = boundedPort(args['preview-port'] ?? '4841', 'preview');
const firstPeerPort = boundedPort(args['peer-port'] ?? '4881', 'PeerJS');
const artifactRoot = resolve(root, 'artifacts/pass71/hf308-chopper-missile');
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!token?.startsWith('--') || !value || value.startsWith('--')) {
      throw new Error(`HF-308 expected --name value; received ${token ?? '(missing)'}`);
    }
    const key = token.slice(2);
    if (!allowed.has(key) || Object.hasOwn(values, key)) throw new Error(`HF-308 rejected --${key}`);
    values[key] = value;
    index += 1;
  }
  return values;
...
function boundedPort(value, label) {
  ...
  if (!Number.isSafeInteger(port) || port < 1_024 || port > 65_535) {
    throw new Error(`HF-308 ${label} port must be from 1024 through 65535`);
  }
  return port;
}
...
function exactComponent(component, expectedScope) {
  ...
  if (!component || typeof component !== 'object' || Array.isArray(component)
    ...
    || component.contract !== 'atomic-acres/pass71-hf308-native-scope-component@1'
    || component.status !== 'passed'
    || component.expectedSourceSha !== expectedSourceSha
    || component.checkoutSourceSha !== expectedSourceSha
    || JSON.stringify({
      arena: component.scope?.arena,
      renderer: component.scope?.renderer,
      mode: component.scope?.mode,
    }) !== JSON.stringify(expectedScope)
    || !Array.isArray(component.scope?.faults)
    || component.scope.faults.length !== 0) {
    throw new Error(`HF-308 component is invalid for ${JSON.stringify(expectedScope)}`);
  }
  return component;
}
...
function focusedOracle(reportPath, startedAt, completedAt) {
  ...
  const missingAssertions = PASS71_HF308_REQUIRED_ASSERTIONS.filter((required) => (
  ...
  if (missingAssertions.length > 0 || testFileCount < PASS71_HF308_MECHANICAL_TEST_FILES.length
    || testCount < PASS71_HF308_REQUIRED_ASSERTIONS.length || passedCount !== testCount || failedCount !== 0) {
    throw new Error(`HF-308 focused oracle is incomplete: ${JSON.stringify({
      missingAssertions, testFileCount, testCount, passedCount, failedCount,
    })}`);
  }
  return {
    contract: 'atomic-acres/pass71-hf308-focused-vitest-oracle@1',
    status: 'passed',
...
async function main() {
  if (!/^[a-f0-9]{40}$/u.test(expectedSourceSha ?? '')) {
    throw new Error('HF-308 requires --expected-source-sha <40 lowercase hex candidate A>');
  }
  if (args.machine !== 'dave-gaming-pc' || process.platform !== 'win32' || process.arch !== 'x64') {
    throw new Error(`HF-308 requires --machine dave-gaming-pc on win32/x64; received ${args.machine}/${process.platform}/${process.arch}`);
  }
  const hostnameSha256 = sha256(Buffer.from(hostname().toLowerCase(), 'utf8'));
  if (hostnameSha256 !== PASS71_HF308_MACHINE_HOSTNAME_SHA256) {
    throw new Error('HF-308 machine hostname does not match dave-gaming-pc');
  }
  ...
  if (checkoutSourceSha !== expectedSourceSha || !clean()) {
    throw new Error(`HF-308 requires one completely clean exact candidate A (${checkoutSourceSha}/${expectedSourceSha})`);
  }
  const allPorts = PASS71_HF308_SCOPES.flatMap((_, index) => [firstPreviewPort + index, firstPeerPort + index]);
  if (new Set(allPorts).size !== allPorts.length || allPorts.some((port) => port > 65_535)) {
    throw new Error('HF-308 owned port set overlaps or exceeds 65535');
  }
  for (const port of allPorts) if (await portIsListening(port)) throw new Error(`HF-308 owned port ${port} is already bound`);
  ...
  if (viteOverrides.length > 0 || inheritedVite.length > 0) {
    throw new Error(`HF-308 rejects Vite overrides: ${[...viteOverrides, ...inheritedVite].join(', ')}`);
  }
  ...
  const inheritedEnvironment = Object.fromEntries(Object.entries(process.env).filter(
    ([name]) => !name.toUpperCase().startsWith('VITE_') && !name.toUpperCase().startsWith('PASS71_HF308_'),
  ));
  ...
  runNode('HF-308 lockfile preflight', ['scripts/qa/verify-npm10-lockfile.mjs'], exactEnvironment);
  runNode('HF-308 evidence contract', [
    '--test', 'scripts/qa/pass71-hf308-chopper-missile-evidence-contract.test.mjs',
  ], exactEnvironment);
  const startedAt = new Date().toISOString();
  const focusedReportPath = join(temporaryRoot, 'focused-vitest.json');
  const mechanicalStartedAt = new Date().toISOString();
  runNode('HF-308 focused Vitest oracle', [
  ...
  const components = [];
  for (const [index, scope] of PASS71_HF308_SCOPES.entries()) {
    const componentPath = resolve(componentRoot, `${scope.arena}-${scope.renderer}-${scope.mode}.json`);
    runNode(`HF-308 ${scope.arena}/${scope.renderer}/${scope.mode}`, [
      ...
      PASS71_HF308_CHOPPER_MISSILE_EVIDENCE: '1',
      PASS71_HF308_EDGE_EXECUTABLE: edgeExecutable,
      PASS71_HF308_EXPECTED_SOURCE_SHA: expectedSourceSha,
      PASS71_HF308_COMPONENT_PATH: componentPath,
      PASS71_HF308_ARENA: scope.arena,
      PASS71_HF308_RENDERER: scope.renderer,
      PASS71_HF308_MODE: scope.mode,
      PASS71_HF308_PEER_PORT: String(firstPeerPort + index),
    });
    if (!existsSync(componentPath)) throw new Error(`HF-308 browser did not emit ${basename(componentPath)}`);
  ...
  if (endingCheckoutSourceSha !== expectedSourceSha || !cleanAfter) {
    throw new Error(`HF-308 source drifted during evidence (${expectedSourceSha}/${endingCheckoutSourceSha})`);
  }
  ...
  if (components.some((component) => JSON.stringify(component.scope.servedCandidate) !== JSON.stringify(servedCandidate))) {
    throw new Error('HF-308 staged candidate identity drifted between scopes');
  }
  if (components.some((component) => component.scope.browser.version !== edgeIdentity.productVersion)) {
    throw new Error('HF-308 browser process and signed executable versions differ');
  }
  ...
  if (components.some((component) => component.scope.browser.launchedExecutablePath !== exactEdgeExecutable)) {
    throw new Error('HF-308 browser process did not bind the exact audited Edge executable');
  }
  const tooling = pass71Hf308ToolingHashesAtSource(root, expectedSourceSha);
  const record = {
    ...PASS71_HF308_CHOPPER_MISSILE_EVIDENCE,
    startedAt,
    ...
    claims: {
      allSupportEnabledArenas: true,
      webgl2AndWebgpu: true,
      offlineAndOwnedHosted: true,
      exactIdentityBinding: true,
      canonicalSixAmmo: true,
      minimumCadence: true,
      exitDeathRematchCleanup: true,
      guestReplicaConvergence: true,
      sameCompletedFrameRasterAttribution: true,
      noSkySpawn: true,
      noDetachedTrail: true,
      signedInstalledEdge: true,
      exactStagedCandidate: true,
      ownerSubjectiveApproval: 'not-claimed',
    },
  };
  record.receiptSha256 = pass71Hf308RecordSha256(record);
  const failures = pass71Hf308EvidenceFailures(record, {
    sourceSha: expectedSourceSha,
    sourceTreeSha,
    tooling,
  });
  if (failures.length > 0) throw new Error(`HF-308 evidence failed: ${failures.join(', ')}`);
  const encodedRecord = Buffer.from(`${JSON.stringify(record)}\n`, 'utf8');
  if (encodedRecord.byteLength > PASS71_HF308_POLICY.maximumEncodedRecordBytes) {
    throw new Error(`HF-308 encoded record is ${encodedRecord.byteLength} bytes; cap is ${PASS71_HF308_POLICY.maximumEncodedRecordBytes}`);
  }
  mkdirSync(artifactRoot, { recursive: true });
  const temporaryPath = `${receiptPath}.tmp`;
  writeFileSync(temporaryPath, encodedRecord);
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    embeddedLosslessPngCount: record.scopes.length * 2,
    rasterRoi: `${PASS71_HF308_POLICY.rasterRoiWidth}x${PASS71_HF308_POLICY.rasterRoiHeight}`,
    encodedRecordBytes: encodedRecord.byteLength,
    maximumEncodedRecordBytes: PASS71_HF308_POLICY.maximumEncodedRecordBytes,
