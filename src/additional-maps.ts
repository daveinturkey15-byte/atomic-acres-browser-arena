...
export function buildSkylineTerminal(scene: THREE.Scene): ArenaMap {
  ...
  for (let z = -10; z <= 30; z += 10) {
    box(builder, 'skyline-tarmac-stripe', [0, 0.027, z], [1.2, 0.03, 4.0], hazardMat, { solid: false, shots: false });
  }
  ...
  for (let seamZ = -16; seamZ <= 32; seamZ += 8) {
    detailBox('apron-marking', `skyline-apron-seam-z-${seamZ}`, [0, 0.028, seamZ], [68, 0.018, 0.035], stainMat);
  }
  for (const [name, x, z, width, depth] of [
    ...
    detailBox('apron-marking', `skyline-aircraft-stand-${name}`, [x, markingY, z], [width, 0.025, depth], hazardMat);
  }
  ...
  for (const [z, rotationY] of [[12, 0.08], [-8, -0.08]] as const) {
    detailBox('apron-marking', `skyline-engine-stain-${z}`, [0, 0.041, z], [3.4, 0.022, 5.2], stainMat, 'performance', [0, rotationY, 0]);
  }
  ...
  detailBox('floor-language', 'skyline-floor-magenta-crossing', [0, 0.095, -20.4], [24, 0.024, 0.16], magentaPracticalMat);
  detailBox('floor-language', 'skyline-floor-window-border', [0, 0.080, -12.55], [59.2, 0.028, 0.52], floorBorderMat);
  ...
  for (let tileZ = -31; tileZ <= -15; tileZ += 4) {
    detailBox('floor-language', `skyline-floor-joint-z-${tileZ}`, [0, 0.084, tileZ], [58.5, 0.018, 0.025], floorBorderMat);
  }
  ...
  for (const lightX of [-18, -10, 0, 10, 18]) {
    detailBox('terminal-story', `skyline-mezzanine-underlight-${lightX}`, [lightX, 3.007, -29.8], [5.4, 0.025, 0.11], practicalMat);
    detailBox('terminal-story', `skyline-mezzanine-underlight-front-${lightX}`, [lightX, 3.007, -24.3], [5.4, 0.025, 0.11], practicalMat);
  }
