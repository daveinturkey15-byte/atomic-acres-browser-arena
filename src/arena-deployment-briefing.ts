...
const BRIEFINGS: Readonly<Record<ArenaId, ArenaDeploymentBriefing>> = Object.freeze({
  ...
  // assert below is deliberately NOT relaxed - the five new arenas get authored
  // copy written from their own `menuLede` in src/map-selection.ts, exactly as
  // the original six were, so no map lands back on a generic loading screen.
  nuketown2: Object.freeze({
    arenaId: 'nuketown2',
    kicker: 'NEIGHBOURHOOD REBUILD',
    briefing: 'Two two-storey houses face each other over 58 m of open road, and the bus in the middle is the only thing between them.',
    approach: 'HELO INBOUND · STREET RUN, BACK YARD TO BACK YARD',
  }),
  raid2: Object.freeze({
    arenaId: 'raid2',
    kicker: 'HILLSIDE MANSION REBUILD',
  ...
  test1: Object.freeze({
    ...
    briefing: 'Sandbag firing lanes run to a two-storey tower, and the container yard flanks every one of them.',
    approach: 'GROUND MOVE · SANDBAG LANES TO THE TOWER',
  }),
  test2: Object.freeze({
    arenaId: 'test2',
    kicker: 'HILLSIDE ESTATE',
    briefing: 'Pool deck above, sunken court below, garden terraces between - three zones, and holding all of them is how you lose.',
    approach: 'HELO INBOUND · GARDEN TERRACE DESCENT',
  }),
  map3: Object.freeze({
    arenaId: 'map3',
    kicker: 'CORRIDOR SHOWCASE',
    briefing: 'Eight corridors run off one paved hub - shoreline, skyline, forest, colonnade - and nothing out here is shooting back.',
    approach: 'WALK IN · HUB FIRST, CORRIDORS IN ANY ORDER',
  }),
