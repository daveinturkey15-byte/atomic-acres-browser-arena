 ...
 *   - the flare gun spawns as skyline-terminal's timed map weapon, and nothing
 *     else grants it;
...
/** The flare gun has exactly one route onto a map: its timed map weapon spawn. */
export function arenaCanAcquireFlareGun(arena: ArenaWeaponReach): boolean {
  return arenaOwnsTimedMapWeapon(arena, 'flare-gun');
}

/** The flamethrower has two: its timed map weapon spawn, and the care package. */
export function arenaCanAcquireFlamethrower(arena: ArenaWeaponReach): boolean {
  return arenaOwnsTimedMapWeapon(arena, 'flamethrower') || arenaCanActivateFieldSupport(arena);
}
