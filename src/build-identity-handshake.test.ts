...
describe('pass95 build-identity lobby handshake', () => {
  // Re-pinned to PASS 95 on 2026-09-02 (the pass95 cut). Same strictness: the stamp must
  ...
  it('stamps the running build for the channel this source publishes to', () => {
    expect(PASS66_RELEASE_IDENTITY.pass).toBe('PASS 95');
    expect(PASS66_RELEASE_IDENTITY.route).toBe('channels/pass95');
    expect(PASS66_RELEASE_IDENTITY.runtimeLabel).toBe('PASS 95');
  });
