...
describe('changelog', () => {
  it('keeps the pending Pass 92 candidate first and freezes every published timestamp behind it', () => {
    expect(CHANGELOG.length).toBeGreaterThan(0);
    const latest = latestChangelogEntry();
    expect(latest.id).toBe('pass92');
    expect(latest.id).toBe(CHANGELOG[0]?.id);
    expect(latest.title).toContain('Pass 92');
    expect(latest.summary).toContain('Pass 92');
    ...
    expect(lastUpdatedButtonLabel(latest)).toBe('PASS 92 · RELEASE CANDIDATE');
    expect(latest.highlights.join('\n')).toContain('Nuke Town: z-fighting removed, stairs wa');
