...
describe('changelog', () => {
  it('keeps the pending Pass 96 candidate first and freezes every published timestamp behind it', () => {
    expect(CHANGELOG.length).toBeGreaterThan(0);
    const latest = latestChangelogEntry();
    expect(latest.id).toBe('pass96');
    expect(latest.id).toBe(CHANGELOG[0]?.id);
    expect(latest.title).toContain('Pass 96');
    expect(latest.summary).toContain('Pass 96');
    ...
    expect(lastUpdatedButtonLabel(latest)).toBe('PASS 96 · RELEASE CANDIDATE');
    expect(latest.highlights.join('\n')).toContain('Nuketown gains analytic sun in-scatter a');
    const pass95Highlights = CHANGELOG.find((entry) => entry.id === 'pass95')?.highlights.join('\n') ?? '';
    expect(pass95Highlights).toContain('House and vehicle materials carry relief');
