...
import {
  ...
  isPass65AuthoredFirearm,
  loadPass65WeaponAsset,
  type Pass65AuthoredFirearmId,
} from './weapon-model';

export type GunRangeRackAsset = Readonly<{
  stationId: string;
  weapon: Pass65AuthoredFirearmId;
  url: string;
}>;

function authoredStationWeapon(stationId: string, weapon: string): Pass65AuthoredFirearmId {
  if (!isPass65AuthoredFirearm(weapon as Pass65AuthoredFirearmId)) {
    throw new Error(`Gun Range station ${stationId} requires an authored firearm, received ${weapon}`);
  }
  return weapon as Pass65AuthoredFirearmId;
}

export const GUN_RANGE_RACK_ASSETS: readonly GunRangeRackAsset[] = Object.freeze(
  GUN_RANGE_WEAPON_STATIONS.map((station) => {
    const weapon = authoredStationWeapon(station.id, station.weapon);
    return Object.freeze({ stationId: station.id, weapon, url: PASS65_AUTHORED_WEAPON_URLS[weapon].world });
  }),
);

export type GunRangeRackPresentationRuntime = Readonly<{
  load(weapon: Pass65AuthoredFirearmId): Promise<void>;
  create(weapon: Pass65AuthoredFirearmId): THREE.Group | null;
  dispose(model: THREE.Group): void;
}>;

export type GunRangeRackLoadOptions = Readonly<{
  recordRequest(url: string): void;
  signal?: AbortSignal;
  runtime?: GunRangeRackPresentationRuntime;
}>;

export type GunRangeRackPresentationReceipt = Readonly<{
  status: 'ready';
  stationCount: number;
...
const defaultRuntime: GunRangeRackPresentationRuntime = Object.freeze({
...
const pendingByRoot = new WeakMap<THREE.Group, Promise<GunRangeRackPresentationReceipt>>();
const readyByRoot = new WeakMap<THREE.Group, GunRangeRackPresentationReceipt>();
...
function stationRoot(arenaRoot: THREE.Group, asset: GunRangeRackAsset): THREE.Group {
  const root = arenaRoot.getObjectByName(`gun-range-weapon-station-${asset.weapon}`);
  if (!(root instanceof THREE.Group)
    || root.userData.stationId !== asset.stationId
    || root.userData.weapon !== asset.weapon) {
    throw new Error(`Gun Range rack identity mismatch for ${asset.stationId}/${asset.weapon}`);
  }
  return root;
}
...
function validateAuthoredModel(model: THREE.Group, asset: GunRangeRackAsset): void {
  ...
  if (model.userData.projectOriginalWeapon !== true
    || model.userData.deliveryVariant !== 'world'
    || source !== asset.url
    || source.includes('procedural')
    || modelId.length === 0
    || modelId.includes('procedural')
    || meshCount === 0) {
    throw new Error(`Gun Range rack rejected non-authored or incomplete model for ${asset.stationId}/${asset.weapon}`);
  }
}
 ...
 */
export function loadGunRangeRackPresentation(
  arenaRoot: THREE.Group,
  ...
  const task = (async () => {
    ...
    for (const root of stationRoots) {
      if (root.children.some((child) => child.name.startsWith('gun-range-rack-weapon-'))) {
        throw new Error(`Gun Range station ${String(root.userData.stationId)} already contains a rack weapon`);
      }
    ...
    const created: Array<{ asset: GunRangeRackAsset; station: THREE.Group; model: THREE.Group }> = [];
    try {
    ...
    const receipt: GunRangeRackPresentationReceipt = Object.freeze({
      status: 'ready',
