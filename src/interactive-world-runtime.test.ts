...
describe('shared interactive-world runtime adapter', () => {
  ...
  it('applies bounded radial explosion damage only under host authority', () => {
    ...
    const hostMutations = host.applyExplosionAt({ origin, radius: 4, maximumDamageQ: 300 });
    expect(hostMutations).toBeGreaterThan(0);
    expect(host.telemetry()).toMatchObject({
      revision: hostMutations,
      presentationRetiredGeometries: 2,
    });
