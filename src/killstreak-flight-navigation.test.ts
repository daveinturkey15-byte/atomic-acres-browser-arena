...
describe('support flight navigation', () => {
  ...
  it('declares arena-owned nav data for every stable arena identity', () => {
    ...
    expect(Object.keys(PASS65_FLIGHT_NAVIGATION).sort()).toEqual([
      ...
      // Graybox wave 2026-09-14 (ShellGray): atomic-acres-rebuild added,
      // sorted right after atomic-acres; world-studio added where it sorts
      // (was missing from this list — the assertion failed before this lane).
      'atomic-acres', 'atomic-acres-rebuild', 'farcrysis', 'gun-range', 'high-seas', 'map3', 'nuketown2', 'raid2', 'rustworks-1v1', 'skyline-terminal', 'test1', 'test2', 'world-studio',
    ]);
