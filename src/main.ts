...
// Pass 65 is one playable application. Renderer selection happens at the
...
try {
  ...
  if (app) {
    ...
    app.innerHTML = `<main id="webgpu-gameplay-blocked"><small>${PASS65_HITL_IDENTITY.pass} · ${PASS65_HITL_IDENTITY.label} · WEBGPU / TSL</small><h1>GAMEPLAY RENDERER BLOCKED</h1><p></p><p>Use <code>?renderer=webgl2</code> only for the explicit rollback-compatible renderer.</p></main>`;
