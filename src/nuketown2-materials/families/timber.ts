...
function sharedTimberGraph(): { colorNode: any; roughnessNode: any } {
  ...
  const variant = NUKETOWN2_UNIFORMS.timberVariant as any;
  ...
  const wood = NUKETOWN2_UNIFORMS.baseColor.mul(wear.albedoMul).mul(float(1).add(boardTone));
...
export function createTimberMaterial(
  ...
  setNuketown2FamilyUniform(mat, 'nuketown2TimberVariant', variant === 'fence' ? 0 : variant === 'deck' ? 1 : 2);
  const shared = sharedTimberGraph();
