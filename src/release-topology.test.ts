...
describe('Pass 89 release topology', () => {
  // Re-pinned from PASS 88 on 2026-09-02 (the pass89 cut). The pass82 publish shipped with
  ...
  it('identifies this source as Pass 89 without moving any protected fallback pin', () => {
    ...
    expect(config.latest.label).toBe('PASS 89');
    ...
    // be the safe backup". PASS 88 is the single pinned backup the in-build chooser
    // (src/bootstrap.ts) falls back to; PASS 73 at channels/the-big-one is retired by the
    // pass89 publish and must no longer be offered anywhere - a card that 404s is worse
    // than no card. Owner policy change, not a weakened pin: the backup is still exact.
    expect(config.pass88Backup).toMatchObject({
  ...
  it('validates the candidate pass identity before it moves anything out of the dist root', () => {
    ...
    // Lane AR (PASS 89): the moves became copy-then-remove so a throw cannot empty the dist root;
...
describe('the published chooser cannot be assembled from two publishes', () => {
  const publish = readFileSync('scripts/orchestration/publish_pass89.py', 'utf8');
  ...
  it('refuses to offer the newest pass with no recent predecessor beside it', () => {
    ...
    // chooser now carries exactly PASS 89 and the pinned PASS 88 backup - not because the
    ...
    expect(publish).toContain('PASS 88 · SAFE BACKUP');
    expect(publish).toContain('KEEP_AT_LEAST = {"pass88"}');
  ...
  it('retires every other tree by enumerating gh-pages at run time and asserts the exact post-state', () => {
    ...
    // is not pass89 or pass88, then look at the disk again before committing.
    expect(publish).toContain('EXPECTED_POST_STATE = {LIVE_TREE, BACKUP_TREE}');
    expect(publish).toContain('LIVE_TREE = "pass89"');
    expect(publish).toContain('BACKUP_TREE = "pass88"');
  ...
  it('keeps the build-freshness guard at exactly the pass88 scope plus the dist-pass89 copy', () => {
    ...
    // pass89 copy of the guard once quietly added `artifacts` to that skip list under a
    ...
    const pass88 = readFileSync('scripts/orchestration/publish_pass88.py', 'utf8');
    ...
    const pass83Set = excluded(guardOf(pass88));
    const pass84Set = excluded(pass84Guard);
    expect(pass83Set.size).toBeGreaterThan(0);
    expect(pass84Set).toEqual(new Set([...pass83Set, 'dist-pass89']));
