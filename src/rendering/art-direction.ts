...
export const ARENA_ART_DIRECTIONS: Readonly<Record<ArenaId, ArenaArtDirection>> = Object.freeze({
  ...
  // Clean neutral training facility: no place-cast at all — the absence of a
  // grade IS its identity next to five loud arenas. The gain/gamma/lift here
  // deliberately CANCEL the quality profile's house teal/warm bias (slope
  // [1.015, 1, 0.98], power [0.995, 1, 1.01], offset [-0.0005, 0, 0.003]) so
  // a neutral grey renders neutral — clinical, bright, unmistakably the lab.
  'gun-range': frozen({
    id: 'gun-range',
