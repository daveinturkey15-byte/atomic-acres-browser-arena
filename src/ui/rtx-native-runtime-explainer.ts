...
/**
 * The visible label of that option in the GRAPHICS MODE select.
 *
 * PASS 89: exported so the three Playwright assertions on the option ladder
 * derive their expectation from the shipped ladder instead of re-typing it.
 * Twice now that list has been re-pinned to a fresh literal and gone stale the
 * next time a rung moved; the ladder itself stays pinned literally, once, in
 * src/graphics-profile-contract.test.ts.
 */
export const RTX_NATIVE_RUNTIME_OPTION_LABEL = 'RTX — WHAT IS IT?';
