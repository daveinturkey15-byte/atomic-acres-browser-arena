...
describe('world-studio house presentation: arena wiring', () => {
  ...
  it('retires the presentation when the arena root leaves the scene', () => {
    const { scene, arena } = harness();
    scene.remove(arena.root);
    expect(arena.root.userData.worldStudioHousePresentation.status()).toBe('disposed');
    expect(arena.root.userData.worldStudioHousePresentation.root.children).toHaveLength(0);
  });
});
