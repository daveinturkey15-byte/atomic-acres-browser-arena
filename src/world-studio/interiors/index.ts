import { BoxGeometry, BufferGeometry, CylinderGeometry, Euler, Group, Matrix4, Mesh, Object3D, Quaternion, Vector3 } from 'three';
...
export function createStudioInteriors(anchors: readonly StudioInteriorAnchor[]): { root: Group; solids: StudioInteriorSolid[] } {
  ...
  // G2 yellow-kitchen dressing: yellow-house ground kitchen+dining (east,
  // centreX +20) only. Exact part mirror of the teal block above (x -> -x
  // about the origin; z shared): identical centers, sizes, roles and names,
  // side-chair yaws negated so every backrest still faces the table
  // (north/south chairs face along z and keep their yaws). One deliberate
  // economy: flat slabs (doors, pulls, rugs, counters — edge rounding under
  // 30 mm is subpixel at review distance) use BoxGeometry at 12 tris instead
  // of RoundedBox at 108, while organic pieces (fruit, foliage) keep
  // RoundedBox and all turnings keep cylinders. A like-for-like 85-part
  // mirror costs 6976 tris against 5732 of headroom left by the teal pass, so
  // the economy is what keeps the frozen 60k interior budget green; centers,
  // sizes, roles and silhouettes match the teal twin exactly. The teal kitchen
  // and all other rooms keep their shipped/G2 sets, so every other room is a
  // control view. The dining pendant and kitchen flush fixtures are
  // lighting-owned (lighting/index.ts FIXTURE_KIND/FOCAL_TOKEN) and already
  // hang over this table and run, so no second shade is added here; the judged
  // plate's circular timber set is the existing dining-table anchor, dressed
  // with chairs, rug and a fruit centerpiece. Every room-scale part below is
  // presentation-only (never solid: zero new solids, zero collision change).
  // Room parts are recorded against the synthetic room anchor
  // `yellow-house-kitchen` (room-local origin [20, 0.08, 0]) rather than a
  // furniture footprint, which they intentionally exceed. Only existing
  // InteriorRoles feed the merge buckets, so the draw-group census never
  // grows. Kitchen scope only: no living/hall/bedroom construction beyond
  // dressing this room.
  if (anchors.some((a) => a.id === 'yellow-house-kitchen-run')) {
    const ROOM = { minX: 20.05, maxX: 26.73, minZ: -8.73, maxZ: 8.73, minY: 0.08, maxY: 3.08 } as const;
    const emitRoom = (name: string, role: InteriorRole, center: V3, size: V3,
      opts: { cylinder?: boolean; radius?: number; yaw?: number } = {}) => {
      const yaw = opts.yaw ?? 0;
      const c = Math.abs(Math.cos(yaw)), s = Math.abs(Math.sin(yaw));
      ...
      if (center[0] - hx < ROOM.minX || center[0] + hx > ROOM.maxX
        || center[2] - hz < ROOM.minZ || center[2] + hz > ROOM.maxZ
        || center[1] - size[1] / 2 < ROOM.minY - 1e-6 || center[1] + size[1] / 2 > ROOM.maxY)
        throw new Error(`Yellow kitchen dressing outside room envelope: ${name}`);
      const roomRotation = new Quaternion().setFromEuler(new Euler(0, yaw, 0));
      const roomRadius = Math.min(opts.radius ?? 0.008, ...size.map((n) => n / 3));
      // Flat-slab economy (see block header): BoxGeometry reads identically at
      // review distance for edge rounding under 30 mm; organic pieces keep
      // RoundedBox and turnings keep cylinders. Same centers, sizes, roles.
      const roomGeometry = opts.cylinder
        ? new CylinderGeometry(size[0] * 0.36, size[0] * 0.5, size[1], 8)
        : (opts.radius ?? 0.008) >= 0.03
          ? new RoundedBoxGeometry(...size, 1, roomRadius)
      ...
      const roomBucket = buckets.get(role) ?? [];
      roomBucket.push(roomNormalized); buckets.set(role, roomBucket);
      components.push({ id: `yellow-house-kitchen-${name}`, anchorId: 'yellow-house-kitchen',
        role, size, local: [center[0] - 20, center[1] - 0.08, center[2]] });
    };
    emitRoom('fridge-body', 'metal', [26.35, 0.99, 2.62], [0.7, 1.82, 0.72]);
    emitRoom('fridge-freezer-door', 'metal', [25.99, 1.58, 2.62], [0.03, 0.6, 0.68]);
    ...
    wallCabinet(6.1, 'upper-north');
    wallCabinet(7.0, 'upper-far-north');
    // Range leg on the north wall with an ochre cabinet west of it, clear of
    // the garage-link door (x 23.5..24.6) by a full cabinet gap.
    emitRoom('stove-body', 'dark', [26.2, 0.49, 8.43], [0.7, 0.82, 0.6]);
    emitRoom('stove-oven-door', 'dark', [26.2, 0.48, 8.12], [0.6, 0.58, 0.03]);
    emitRoom('stove-handle', 'metal', [26.2, 0.8, 8.1], [0.5, 0.04, 0.05]);
    emitRoom('stove-hob', 'metal', [26.2, 0.915, 8.43], [0.7, 0.03, 0.6]);
    for (const x of [-0.18, 0.18]) for (const z of [-0.15, 0.15])
      emitRoom(`stove-burner-${x}-${z}`, 'dark', [26.2 + x, 0.94, 8.43 + z], [0.18, 0.02, 0.18], { cylinder: true });
    emitRoom('cabinet-west-carcass', 'walnut', [25.35, 0.49, 8.43], [0.8, 0.82, 0.6]);
    emitRoom('cabinet-west-door', 'ochre', [25.35, 0.49, 8.12], [0.76, 0.74, 0.02]);
    emitRoom('cabinet-west-pull', 'metal', [25.35, 0.62, 8.1], [0.2, 0.03, 0.03]);
    emitRoom('cabinet-west-counter', 'porcelain', [25.35, 0.945, 8.43], [0.84, 0.05, 0.6]);
    // Countertop dressing on the existing run (counter top y 0.9175).
    emitRoom('counter-fruit-bowl', 'ochre', [26.2, 0.96, 6.6], [0.3, 0.08, 0.3], { cylinder: true });
    emitRoom('counter-fruit-0', 'ochre', [26.26, 1.02, 6.58], [0.09, 0.09, 0.09], { radius: 0.03 });
    emitRoom('counter-fruit-1', 'cream', [26.14, 1.02, 6.62], [0.09, 0.09, 0.09], { radius: 0.03 });
    ...
    emitRoom('dining-rug-underlay', 'cream', [23.4, 0.09, -3.4], [3.2, 0.02, 2.6], { radius: 0.004 });
    emitRoom('dining-rug-pattern', 'quilt', [23.4, 0.105, -3.4], [3.0, 0.015, 2.4], { radius: 0.003 });
    // Four dining chairs around the existing table (top y 0.785); side chairs
    // carry negated yaws so their backrests still face the table.
    const diningChair = (cx: number, cz: number, yaw: number, tag: string) => {
      const c = Math.cos(yaw), s = Math.sin(yaw);
      const at = (lx: number, lz: number): [number, number] => [cx + lx * c + lz * s, cz - lx * s + lz * c];
      for (const x of [-0.18, 0.18]) for (const z of [-0.18, 0.18]) {
    ...
    diningChair(24.55, -3.4, -Math.PI / 2, 'chair-east');
    diningChair(22.25, -3.4, Math.PI / 2, 'chair-west');
    diningChair(23.4, -4.45, 0, 'chair-north');
    diningChair(23.4, -2.35, Math.PI, 'chair-south');
    // Fruit centerpiece on the table, clear of the anchor's book.
    emitRoom('table-fruit-bowl', 'ochre', [23.4, 0.82, -3.4], [0.34, 0.07, 0.34], { cylinder: true });
