...
test('chooses the latest build before the normal menu loads', async ({ page }) => {
  ...
  await expect(page.locator('[data-release-choice="stable"]')).toContainText('PASS 59');
  await expect(page.locator('[data-release-choice="latest"]')).toContainText('LATEST APPROVED');
  await expect(page.locator('[data-release-choice="latest"]')).not.toContainText('PASS 59');
