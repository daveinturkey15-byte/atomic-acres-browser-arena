...
test('offers Pass 72, exact previous Pass 70, retained Pass 69 and stable Pass 63 WebGL', async ({ page }, testInfo) => {
  ...
  expect(releaseChannels.latest.label).toBe('PASS 72');
  expect(releaseChannels.experimental.label).toBe('PASS 72');
  ...
  await expect(page.locator('[data-release-choice="retained"]')).toContainText('PREVIOUS STABLE');
  ...
  const artifactRoot = resolve(process.cwd(), 'artifacts/pass72/release-shell');
  mkdirSync(artifactRoot, { recursive: true });
  const screenshot = resolve(artifactRoot, 'pass72-pass70-pass69-pass63-chooser.png');
  await page.screenshot({ path: screenshot, animations: 'disabled', fullPage: true });
  await testInfo.attach('pass72-pass70-pass69-pass63-chooser', { path: screenshot, contentType: 'image/png' });
...
test('front-page hard reset clears CacheStorage and reloads the chooser', async ({ page }) => {
  ...
  await page.evaluate(async () => { await caches.open('pass72-stale-test'); });
  await page.locator('[id$="hard-refresh"]').click();
  await page.waitForURL(/cachebust=\d+/u);
  await expect(page.locator('#release-channel-gate')).toBeVisible();
  expect(await page.evaluate(async () => (await caches.keys()).includes('pass72-stale-test'))).toBe(false);
});
...
test('routes the previous-live choice to exact Pass 70', async ({ page }) => {
  await page.goto('/?release=choose');
...
test('routes the retained choice to exact Pass 69', async ({ page }) => {
  await page.goto('/?release=choose');
...
test('keeps legacy latest, normal and room entries on Pass 72', async ({ page }) => {
