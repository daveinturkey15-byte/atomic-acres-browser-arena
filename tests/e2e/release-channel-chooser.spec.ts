...
test('offers live Pass 69 The Big One, stable Pass 67.1 and rollback Pass 63 before the menu loads', async ({ page }, testInfo) => {
  ...
  expect(releaseChannels.latest.label).toBe('THE BIG ONE v69.2');
  expect(releaseChannels.experimental.label).toBe('THE BIG ONE v69.2');
...
test('front-page hard reset clears CacheStorage and reloads the chooser', async ({ page }) => {
  ...
  await page.evaluate(async () => { await caches.open('pass69-2-stale-test'); });
  await page.locator('[id$="hard-refresh"]').click();
  await page.waitForURL(/cachebust=\d+/u);
  await expect(page.locator('#release-channel-gate')).toBeVisible();
  expect(await page.evaluate(async () => (await caches.keys()).includes('pass69-2-stale-test'))).toBe(false);
});
