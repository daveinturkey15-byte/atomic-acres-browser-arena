-- RAID2, Pass 87 (owner 2026-09-02, HF-408; renumbered 0008 -> 0009 at
-- integration because 0008 was already taken by the Nuke Town Rebuild): the Raid layout rebuild joins the
-- canonical arena set. Keep the D1 persistence boundary aligned with the
-- eleven-arena diagnostics schema. SQLite cannot alter a CHECK constraint in
-- place, so rebuild the table without discarding retained diagnostic rows
-- (same pattern as 0005, 0006, 0007 and 0008).
--
...
CREATE TABLE match_diagnostics_v6 (
  ...
  arena TEXT NOT NULL CHECK(arena IN ('atomic-acres', 'skyline-terminal', 'rustworks-1v1', 'gun-range', 'farcrysis', 'high-seas', 'test1', 'test2', 'map3', 'nuketown2', 'raid2')),
...
INSERT INTO match_diagnostics_v6 (
  receipt_id,
  idempotency_key,
  received_at,
  expires_at,
...
ALTER TABLE match_diagnostics_v6 RENAME TO match_diagnostics;
